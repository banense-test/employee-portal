...
| **Regression baseline (15 executed PASS)** | ci.yml @ 5a2ba38 merge-sequence record (PR #3 → 33617283642; PR #5 → 33617446626; PR #4 → 33617748483 — every merged PR re-ran ALL prior suites, GREEN); mandatory regression policy (Test Evaluation Summary) | DependsOn | Every subsequent merged PR (re-runs all 15); Construction regression suite baseline |
...
| Test Evaluation Flow diagram (formal pass, § Test Scope) | S2–S4 this revision (smoke gate; execute and evaluate R001 > R003 > R004; verdict distribution; defect census) | Refines | Findings — Cycle 1 formal-pass record; Test Evaluation Summary mission verdict |
| Test Case Lifecycle diagram (Iter 3, § Test Scope) | F-CR-E1-1 RESOLVED (Review Record Iter 3); merged mechanism code (file shas above); Construction scheduling for TC-003/TC-010/TC-013…TC-018 | Refines | Per-case state transitions (Cycle 1 formal-pass record); regression policy (mandatory re-run after every merged PR) |
