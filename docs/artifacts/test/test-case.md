...
**[OMITTED: Test Plan — trigger not fired per Development Case §5.2 oracle (consulted this iteration); per-iteration testing scope lives in the Iteration Plan and the Test Evaluation Summary.]**
...
| Functionality | One clocking event per press; status-aware button; six directory attributes displayed; unpublish = soft delete with record retained | FR-004, FR-010, FR-009, CON-012 | TC-001…TC-003, TC-009…TC-011, TC-013…TC-016 |
...
| Data Integrity / Auditability | Timestamp captured at press, stored UTC, persisted unchanged on sync; audit entries append-only, atomic with the state change | DAT-001, DAT-002, NFR-005, AUD-003 | TC-004, TC-005, TC-008, TC-013, TC-016 |

### Test Types Mapped to Quality Dimensions and Levels

| Test Type | Dimension | Level | Cases |
|---|---|---|---|
| Functional (mechanism validation) | Functionality | System + Integration | TC-001…TC-003, TC-009…TC-011, TC-013…TC-016 |
...
| Data integrity / audit | Auditability | Unit + Integration | TC-008, TC-016 |
| Regression | All | All | every case — re-run after EVERY merged PR (mandatory policy, Test Evaluation Summary) |

Multi-level coverage is deliberate: unit level (FakeClock DST boundary, audit atomicity), integration level (stub issuer, disposable directory, real PostgreSQL), system level (browser flows with network control). No level is skipped; no level is exclusive.
...
title Test Automation Architecture - Elaboration Iteration 1\nStubs, drivers, fixtures and fakes for UC-001 / UC-004 / UC-010
...
note bottom of LDAPFIX
  ...
  directory, NOT production AD. Answers
  R001's data-shape question; missing
  attributes render blank, entry NOT
  hidden. Production fidelity = R011
  (Construction).
end note
...
**Stub/driver justification (testability contract):** the stub OIDC issuer and disposable LDAP directory exist because the stakeholder explicitly refused to wait on STK-004 (R010) — R003 is proven by consuming tokens correctly, not by how the issuer got its users; R001 is a data-shape question answered by a disposable directory. The PostgreSQL dev instance is the REAL declared engine because idempotency (`ON CONFLICT`), the UNIQUE constraint, and the append-only REVOKE are engine semantics an in-memory fake cannot reproduce. The unit-level fakes exist because the Design Model made every subsystem boundary an interface (INT-006…INT-019) — the testability entry points are consumed here, not re-invented.

### Test Workflow — UC Scenario to Executable Test Case

```plantuml
@startuml
title Test Workflow - UC Scenario to Executable Test Case (Elaboration Iteration 1)
...
```plantuml
@startuml
title Test Case Lifecycle (TC-001..TC-020) - Elaboration Iteration 1
...
| TC-011 | UC-004 AF-2 / R001 — attribute population + graceful degradation | Integration | Functional (risk validation) | API driver + disposable LDAP | 1 (R001, HIGH) |
...
**Coverage check:** UC-001 — main (TC-001/002), AF-1 (TC-004/005/006/020), AF-2 (TC-007), AF-3 (TC-003), timestamp convention (TC-008). UC-004 — main (TC-009), AF-1 (TC-010), AF-2 (TC-011), AF-3 (TC-012). UC-010 — main (TC-013), AF-1 (TC-014), AF-2 (TC-015), audit/soft-delete invariants (TC-016). Cross-cutting: SEC-006 (TC-017), SEC-007 (TC-018), R003/SEC-001/002/003 (TC-019). Every flow of the three priority UCs has at least one adversarial case; no case exists without a UC or declared-mechanism trace.
...
**TC-011 — UC-004 AF-2 / R001: attribute population + graceful degradation (Integration, risk validation — HIGH)**

| Field | Value |
|---|---|
| Traces | UC-004 AF-2; FR-010; R001 acceptance criteria (Risk List: >90% per office); USA-003; SEQ-004 |
| Preconditions | Disposable LDAP directory loaded with DELIBERATE attribute gaps (§ Test Data: O1 one entry missing extension; O2 one missing job title; O3 fully populated) |
| Input data | One search per office (department = O1 / O2 / O3), returning all 20 entries each |
| Procedure | See procedure diagram below |
| Expected outcome | Six attributes populated for **> 90%** of sampled users per office (O1: 19/20 = 95%; O2: 19/20 = 95%; O3: 20/20 = 100%); entries with missing attributes render the missing field BLANK and the entry is **NOT hidden** (graceful degradation); no cross-mapped attributes; per-office population rates recorded as R001 empirical evidence |
| Pass criteria | ASSERT-1…ASSERT-4 hold; population rates > 90% per office |
| Failure scenario attacked | R001's declared failure mode: entries with missing attributes hidden entirely (the directory shows gaps), or population below 90% in some office — the exact HIGH risk this validation retires empirically |
| Automation + interface points | API driver + disposable LDAP; INT-010 ILdapGateway.Search + MapEntry; fixture: disposable LDAP directory (NOT production AD — production fidelity is R011, Construction) |

**Test Procedure — TC-011 (R001, disposable directory):**

```plantuml
@startuml
title Test Procedure - TC-011: R001 LDAP Attribute Validation (HIGH risk, disposable directory)

start
:Fixture: disposable LDAP directory - 60 synthetic\nentries, 3 offices (O1/O2/O3, 20 each),\ndeliberate gaps (O1: 1 missing extension;\nO2: 1 missing job title; O3: fully populated);
:Query per office via CLS-009 LdapGateway\n(INT-010 ILdapGateway.Search, LDAP v3 read-only);
if (Query succeeds within 5 s (PRF-003)?) then (yes)
  :Map results to CLS-026 DirectoryEntry\n(six corporate fields, FR-010);
  :ASSERT-1 population: six attributes populated\nfor >90% of sampled users per office\n(R001 criterion: O1 95%, O2 95%, O3 100%);
  if (Entry carries missing attributes?) then (yes - UC-004 AF-2)
    :ASSERT-2 missing fields render BLANK;\nthe entry is NOT hidden (graceful degradation);
  else (no)
    :ASSERT-3 all six fields display on\nthe person card (USA-003, no detail view);
  endif
  :ASSERT-4 no cross-mapped attribute\n(e.g. extension rendered as email);
else (no - UC-004 AF-3 path)
  :ASSERT-5 hard timeout at 5 s;\n"Directory temporarily unavailable";\nno local fallback (CON-006);
endif
:Record per-office population rates as R001\nempirical evidence (LCA evidence package);
stop
@enduml
```
...
|---|---|---|---|
| E-001 | e001 | Employee | directory search actor (TC-009, TC-010, TC-013 step 6) |
...
| HR-001 | hr001 | HR Administrator | news management actor (TC-013…TC-016) |

**Token variants (TC-007, TC-017, TC-018, TC-019):** V1 valid Employee (roles: ["employee"]); V2 valid HR Administrator (roles: ["employee","hr-administrator"]); V3 expired (valid signature, exp in the past); V4 bad signature (valid structure, wrong signing key); V5 absent (no Authorization header). Role claim names are the stub's configuration — the test asserts the PORTAL maps whatever claims the issuer sends (SEC-002), not a hardcoded claim path.

### Disposable LDAP Directory (R001 fixture — TC-009…TC-012)

60 synthetic entries, 3 offices (O1/O2/O3), 20 each. Six corporate attributes per entry (FR-010): displayName, title, department, office, mail, telephoneNumber (extension).

| Office | Entries | Deliberate gap | Population rate |
|---|---|---|---|
| O1 | 20 | `ldap-o1-019` missing telephoneNumber | 19/20 = 95% (> 90% ✓) |
| O2 | 20 | `ldap-o2-007` missing title | 19/20 = 95% (> 90% ✓) |
| O3 | 20 | none — fully populated | 20/20 = 100% |

Named search target: "Gómez, Elena" (`ldap-o1-003`, fully populated, department=O1) — the TC-009 happy-path hit. The fixture also carries one entry with an empty-string attribute (distinct from absent) to exercise the null-vs-empty mapping edge in CLS-009 MapEntry. **This is NOT production AD** — production-instance fidelity is risk R011, retired in Construction integration (R010).
...
`worker-categories.json` (ADR-004) test copy: ["Administrative", "Technical", "Operational"] — a representative FIXED list; no CRUD path exists anywhere in the portal (CON-013), so the fixture only needs to load and be read.

## Traceability

| Element | Traces From | Link Type | Traces To |
|---|---|---|---|
| Test Case artifact (this) | Use-Case Model (UC-001, UC-004, UC-010 — test priority 1); Supplementary Specification (PRF-002/003, REL-002/003, SEC-001/002/003/006/007, AUD-003, DAT-001/002, USA-003/008); SAD (COMP-001…011, ADR-003/004); Design Model (CLS-001…027, INT-006…019, testability entry points); Test Evaluation Summary (Evaluation Mission R001 > R003 > R004); Review Record (F-CR-E1-1, actions A-1…A-6) | Tests | Elaboration exit criteria 1–3; LCA evidence package; Construction regression suite baseline |
...
| TC-011 | UC-004 AF-2, FR-010, R001 acceptance criteria (Risk List), USA-003 | Tests | CLS-009 MapEntry, INT-010; COMP-007; disposable LDAP fixture (R001 empirical evidence) |
