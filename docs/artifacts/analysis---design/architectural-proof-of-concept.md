...
| Iteration | 5 (Cycle 1) — citation-correction pass (**action A-38, closing PoC F3**): the § Traceability behavioural-bar row's CONTRIBUTING.md sha citation corrected to the verified current file sha **90e4f2e1b91bdb64082dcc9f75a4b32c3cc10f80** (read first-hand via scm_get_file_content on main, Elab Iter 5; ARCH-6 carries the four-clause behavioural bar verbatim — the substantive claim was already verified TRUE; the citation itself was the defect). **SCM Issue #9 — the CR targeting this artifact — CLOSED on the verified A-32 evidence** (cr:complete, executor: Software Architect) per the stakeholder's folded verdict-gate directive, verbatim: "Close all findings and issues opened". Prior revisions preserved: Iter 4 (record-propagation pass, action A-32 — the observed results ledger), Iter 3 (validation protocol + FOUR-clause behavioural bar + 23-case enumeration), Iter 2 (protocol + per-risk dispositions) |
| Date | 2026-09-02 |
...
1. **The mechanisms are the Construction baseline.** COMP-007, COMP-006, and COMP-009 are built as production code in `src/` (Services/ and Infrastructure/ per the Implementation View) — the PoC does not create a parallel throwaway tree. What the validation exercises is what Construction inherits. **Observed (Iter 3):** all three mechanisms are merged to `main` via the baseline-close PR #6 (review state APPROVED) — the Construction baseline IS the validated code.
2. **R001's retirement is behavioural, not statistical — and blankness is a contract, not a rendering accident.** The graceful-degradation policy (missing attribute = blank field, entry NOT hidden, no error, **never a substituted value**) is the architectural guarantee; it is shared by all four AD-reading UCs through the single LDAP read path (COMP-007 → COMP-003 → IDIR consumers). The fourth clause makes blankness an explicit prohibition on substitution: no default, no placeholder, no guessed value, no another-employee's value — the CSV that reaches payroll must carry an empty cell that gets questioned, never a plausible wrong department. **Observed (Iter 3):** the four-clause contract holds clause-by-clause across all four consumers (TC-011 + TC-021/022/023 PASS, clause (d) against the substitution-attempt fixtures) and is implemented in code (`LdapGateway.cs` sha b8df8b7 — "missing or empty AD value → null; null is the FINAL mapped value"). The statistical measurement of the real AD's data quality is a Construction activity with R010 delivery — excluded from the LCA evidence package.
3. **R010 does not gate Elaboration.** The disposable directory and stub issuer remove the STK-004 dependency from this phase's validation — **observed**: the validation ran to completion without any STK-004 deliverable. Production-instance integration (real service account, real Keycloak client registration) is Construction work; its residual is R011.
4. **R011 is the accepted residual.** The fixtures may differ from the production instances (attribute schemas, claim shapes). Both COMP-007 and COMP-006 are High-volatility encapsulations by design (SAD Volatility Analysis), so a Construction-time adjustment is contained to one component each. The fixtures are retained as reusable Construction test assets — the materialized 11-entry disposable-LDAP fixture (sha 5b84206) and the stub OIDC issuer carry every gap shape and substitution temptation forward.
5. **No structural change to the baseline resulted from the observed validation (Iter 3).** The 4+1 structure, the 11-component decomposition, and the interface boundaries are unchanged. The one documented deviation is the interim in-memory `ClockingsRepository` — a test-seam realization of INT-016's contract whose PostgreSQL adapter lands Construction Iteration 1 per R008 (Code Reviewer finding F-CR-E3-1, `[DEFERRED]` marker carried in the PR record); the REL-002 UNIQUE-key contract itself is enforced and validated at the seam. Had a validation failure implied structural change, it would have been recorded as a new finding against the SAD — none arose.
## Traceability
...
| R003 validation (stub OIDC issuer) | R003; CON-004; SEC-001, SEC-002, SEC-006 | Tests | COMP-006 / CLS-010 (src/); TC-007, TC-019 (token-validation matrix) |
| R004 validation (5-minute drop simulation) | R004; NFR-004; AC-005; REL-002, REL-003; PRF-002; ADR-003 | Tests | COMP-009 / CLS-008 (src/); COMP-001 sync endpoint; COMP-008 UNIQUE idempotency_key; TC-001, TC-004/005/006, TC-020 |
| Behavioural bar (R001 acceptance criteria — FOUR clauses) | Stakeholder decision (Elab Iter 2): three clauses, confirmed for UC-004/005/006/007; stakeholder verdict-gate contribution (Elab Iter 2, verbatim): "a missing attribute is displayed as missing. It is never replaced by a default, a placeholder, a guessed value, or another employee's value" | Authorizes | Disposable-directory fixture specification (deliberate gap seeding + substitution-attempt fixtures); SAD § Quality PoC Plan (four-clause record, action A-31); Design Model P-05/CLS-009 (action A-27); Test Case TC-011 + TC-021/022/023 fourth-clause steps (action A-28); CONTRIBUTING.md ARCH-6 (fourth clause, action A-36 — file sha 90e4f2e1b91bdb64082dcc9f75a4b32c3cc10f80 on main, verified via scm_get_file_content Elab Iter 5; citation corrected per action A-38, closing PoC F3) |
| Per-risk dispositions (single-mechanism ×3) | record_poc_decision (R001, R003, R004 — Iter 2; R001 re-recorded with the four-clause bar — Iter 3) | Specifies | Implementer actions A-2…A-4 (feature/E1-{risk-id} branches — DELIVERED); Code Reviewer CR-1…CR-7 (3 APPROVED dispositions); Integrator merge to iteration/E1 (DONE) |
| **Observed results ledger (§ Results and Findings, Iter 4 — action A-32)** | Test Case Cycle 1 formal-pass record (the evidence authority: 15 PASS · 0 FAIL · 8 BLOCKED, execution trace CI run 33617748483); merged PRs #3/#4/#5 (APPROVED ×3, reviews 5088169328/5088169517/5088169685) + PR #6 (main, APPROVED); Issue #1 CLOSED cr:complete; main CI GREEN run 33620993027 | Derives | R6 evidence package (the gate's core artifact); PM close-pass risk-retirement recording (Work Item 11 — R001/R003/R004 → RETIRED); SAD § Quality LCA criterion 3 (action A-33); Test Evaluation Summary mission verdict (Test Manager, A-35) |
| **R001 observed evidence (FOUR clauses × FOUR consumers PASS)** | TC-011 + TC-021/022/023 clause-by-clause table (Test Case authority); `BehaviouralBarTests` @ 3660454, `LdapGatewayTests` @ caec358, `ReportExportServiceTests` @ 4bbb732, `DirectoryServiceTests` @ 06c5bd4 → CI run 33617748483; materialized fixture `Fixtures/DisposableLdapDirectory.cs` @ 5b84206 | Tests | COMP-007 / CLS-009 (src/, `LdapGateway.cs` sha b8df8b7); R001 retirement record (Risk List, PM close-pass); exit criterion 1 |
| **R003 observed evidence (token matrix PASS)** | TC-019 + TC-007 (`KeycloakAuthProviderTests` @ 3a63509 → CI run 33617748483): redirect flow, JWKS validation, verbatim role extraction, 10 rejection variants at the request boundary | Tests | COMP-006 / CLS-010 (src/, sha 7bd4cfd); R003 retirement record (Risk List, PM close-pass); exit criterion 2 |
| **R004 observed evidence (drop simulation PASS)** | TC-001, TC-004/005/006, TC-020 (`OfflineResilienceTests` @ 0a7b1a2, `OfflineQueueTests` @ cb4b843, `ClockingServiceTests` @ 58476ca → CI run 33617748483): < 1 s both paths, zero losses, ≤ 60 s sync, duplicates rejected, capacity ≥ 10 | Tests | COMP-009 / CLS-008 (src/); AC-005; R004 retirement record (Risk List, PM close-pass); exit criterion 3 |
| **8 BLOCKED scope decision (framing directive, binding)** | Stakeholder Iter 3 directive, verbatim: "the 8 BLOCKED test cases are a recorded SCOPE decision (production AD and Keycloak integration belongs to Construction), not an open gap. State it that way in the evidence package so the LCA reads them as deferred, not as missing."; Test Case honest-corrections record (TC-003/TC-010/TC-017/TC-018 Scripted → BLOCKED; TC-013…TC-016 Construction scope) | Refines | R6 evidence-package framing (15 executed PASS + 8 deferred-by-scope-decision, zero FAIL); Construction execution of TC-003/TC-010/TC-013…TC-018; R010/R011 Construction residuals |
...
| Delivery protocol | BRANCHING_STRATEGY §5.2, invariants 8.1/8.2/8.4; Review Record F-CR-E1-1 remediation; Test Case § Test Case Catalog (23-case authority — PoC F1 correction, action A-21) | Refines | feature/E1-R001, feature/E1-R003, feature/E1-R004 branches (DELIVERED); PRs #3/#4/#5 to iteration/E1 + PR #6 to main (MERGED); TC-001…TC-023 execution (COMPLETE) |
