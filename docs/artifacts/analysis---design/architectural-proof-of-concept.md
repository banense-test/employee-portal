...
|---|---|
| Phase | Elaboration |
| Status | Draft — validation protocol established; empirical execution IN PROGRESS this convergence cycle (Elaboration Iteration 2). Mechanism code delivery (Implementer, actions A-2…A-4) and TC-001…TC-020 execution are the cycle's remaining work; empirical results are recorded in § Results and Findings as they land — no result is claimed before it is observed |
| Milestone Target | End of Elaboration (LCA) — this artifact is the core of the LCA evidence package |
| Iteration | 2 (Cycle 1) |
| Date | 2026-09-01 |
...
| Risk | Mode | Rationale |
|---|---|---|
| R001 | **single-mechanism** | One mechanism is clearly right (COMP-007 LDAP Gateway with graceful degradation) but must be proven by running code against deliberately-gapped data |
...
No candidate competition exists for any of the three risks — each mechanism was decided by ADR (ADR-003 for R004) or by the declared constraints (CON-004/005 for R003/R001); what remains is empirical proof, not mechanism selection.
...
title Employee Portal — PoC Validation Architecture (Elaboration Iter 2 — empirical risk retirement)
...
package "Validation fixtures — disposable, Elaboration only" as FIX {
  component "Disposable LDAP Directory\n3 offices seeded;\nattribute gaps seeded DELIBERATELY" as DLDAP
...
note bottom of DLDAP
  ...
  Bar is BEHAVIOURAL (stakeholder,
  Elab Iter 2): every employee rendered;
  a missing attribute never removes
  someone from search results; a missing
  attribute never raises an error.
...
**R001 — behavioural bar (stakeholder decision, Elab Iter 2; the unsourced >90% statistical figure is DROPPED):**
...
**Delivery protocol (BRANCHING_STRATEGY §5.2):** the Implementer builds each mechanism in `src/` on `feature/E1-{risk-id}` branches based on `iteration/E1`, ships dual-coverage unit tests (black-box contract + white-box paths), and labels each branch `ready-for-review`; the Code Reviewer opens one PR per branch (base `iteration/E1`) and applies CR-1…CR-7 with terminal dispositions; the Integrator merges APPROVED PRs. The Test Designer executes TC-001…TC-020 against the validation fixtures. Empirical results feed this artifact's § Results and Findings.

## Results and Findings

**Status as of 2026-09-01 (honest ledger — no result is claimed before it is observed):**

| Item | Status | Evidence |
|---|---|---|
| Per-risk dispositions recorded (single-mechanism ×3) | **DONE** | `record_poc_decision` executed for R001, R003, R004 (this iteration) |
| Validation protocol + acceptance criteria established | **DONE** | § Approach (this artifact); SAD § Quality PoC Plan corrected to the empirical disposition (SAD F1 resolved) |
| R001 mechanism code (COMP-007/CLS-009 + disposable directory fixture) | **PENDING — Implementer (A-2)** | `iteration/E1` verified skeleton-only (no `Services/`, no `Infrastructure/`, no LDAP packages); zero PRs in any state |
| R003 mechanism code (COMP-006/CLS-010 + stub issuer fixture) | **PENDING — Implementer (A-3)** | Same SCM state |
| R004 mechanism code (COMP-009/CLS-008 + drop simulation) | **PENDING — Implementer (A-4)** | Same SCM state |
| TC-001…TC-020 execution | **PENDING — Test Designer** | All 20 cases BLOCKED on mechanism delivery (Test Case Cycle 1 record; SCM Issue #1) |
| Empirical results R001 / R003 / R004 | **PENDING** | No empirical result exists yet. This section is evolved with observed results as the mechanisms land — pass/fail recorded per acceptance criterion, never projected |

**Interim finding (design-level, already actionable):** the R001 behavioural bar changes what the disposable-directory fixture must contain — gaps must be seeded **deliberately** (missing job title / extension / email / department / office across all 3 offices) so each of the three behavioural clauses can actually fail. A uniformly-populated fixture would pass vacuously and prove nothing. This requirement is now part of the fixture specification the Implementer builds against.

**What this artifact does NOT claim:** it does not claim R001/R003/R004 are retired. Retirement is recorded only when the empirical results land and the acceptance criteria are observed to hold. The LCA re-presentation requires this section to carry those observed results.
...
2. **R001's retirement is behavioural, not statistical.** The graceful-degradation policy (missing attribute = blank field, entry NOT hidden, no error) is the architectural guarantee; it is shared by all four AD-reading UCs through the single LDAP read path (COMP-007 → COMP-003 → IDIR consumers). The statistical measurement of the real AD's data quality is a Construction activity with R010 delivery — excluded from the LCA evidence package.
...
| R001 validation (disposable LDAP directory) | R001 (declared, P=3, I=3, exposure=9); FR-010; CON-005, CON-006, CON-007; stakeholder behavioural bar (Elab Iter 2) | Tests | COMP-007 / CLS-009 (src/); UC-004 AF-2, UC-005, UC-006, UC-007 derived clauses; TC-011 and related cases |
...
| Behavioural bar (R001 acceptance criteria) | Stakeholder decision (Elab Iter 2): three clauses, confirmed for UC-004/005/006/007 | Authorizes | Disposable-directory fixture specification (deliberate gap seeding); SAD § Quality PoC Plan |
| Per-risk dispositions (single-mechanism ×3) | record_poc_decision (R001, R003, R004 — this iteration) | Specifies | Implementer actions A-2…A-4 (feature/E1-{risk-id} branches); Code Reviewer CR-1…CR-7; Integrator merge to iteration/E1 |
...
| Delivery protocol | BRANCHING_STRATEGY §5.2, invariants 8.1/8.2/8.4; Review Record F-CR-E1-1 remediation | Refines | feature/E1-R001, feature/E1-R003, feature/E1-R004 branches; PRs to iteration/E1 |