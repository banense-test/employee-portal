...
| INT-011 | IAuthProvider | BuildAuthorizeRedirectUrl(redirectUri, state) — **added Iter 4 (F-CR-E3-2)** | redirectUri non-empty; state non-empty (the CSRF parameter — generated per challenge by the middleware) | Returns the issuer's authorize URL (client_id, redirect_uri, response_type=code, state) — the portal's half of the CON-004 redirect flow (SEQ-001, at the middleware); round-trip state validation is Construction scope (session mechanism) — the Elaboration contract generates the state and does not claim its validation |
