...
| INT-011 | IAuthProvider | GetAuthenticatedUser(context) — **row updated Iter 4 to the merged async signature** | — | AuthenticatedUser (Uid + Roles) or null if unauthenticated; throws OidcTokenValidationException when the presented token is expired or invalid — rejected at the boundary (401), never passed downstream |
