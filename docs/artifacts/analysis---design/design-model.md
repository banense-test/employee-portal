...
|---|---|
| Phase | Elaboration |
| Status | **Approved** — Elaboration LCA reviews (Reviewer-lens per-artifact verdicts: Approved at Iters 3/4/5/6; zero findings ever emitted against this artifact; the artifact-level status in the review inventory is Approved). Corrected at Iter 6 from the stale "Draft" set at artifact creation and never updated across five review-approved revisions — the R014 record-propagation class (status honesty in both directions: understating review state is as dishonest as overstating delivery — the F7 lesson applied to this artifact's own record). |
| Milestone Target | End of Elaboration (LCA) |
| Iteration | 7 (Cycle 1) — preservation pass (Designer). This revision: §Document Control updated in place ONLY — the R014 post-write staleness subclass corrected in this artifact's own record (the same class the Reviewer minted UCM F1/SUP F1 for at Iter 6: a record accurate at its write time, staled by the same cycle's later events). The Iter 6 record's findings-ledger enumeration (open: TES F4 → A-40; DC F5 → A-41; F-CR-E3-2 open) no longer holds: TES F4 and DC F5 were ledger-closed 2026-09-03 (Reviewer lens, Iter 6 record); F-CR-E3-2 was closed by the Code Reviewer lens at Iter 6; the cycle minted five successor findings (TES F5, DC F6, UCM F1, SUP F1, TC F2 → actions A-42…A-46, all Minor, all owned by Test Manager / Process Engineer / System Analyst / Tester — none targets this artifact). Per the stakeholder's Iter 6 verdict-gate directive — the remaining-work state "already has an owner in the Review Record findings ledger and in the SCM issues… a document that copies someone else's state will drift from it forever" — this record no longer enumerates other artifacts' open findings; it references the two owning registries: the Review Record findings ledger (single source of truth, verified via the findings system at each review) and the SCM issue tracker (live census verified this iteration via scm_list_issues: 1 open CR vehicle — Issue #14, the CR vehicle for the ledger-closed TES F4/A-40, assigned:test-manager, cr:approved; 5 closed — #1/#2/#9/#11/#12, all cr:complete). This artifact's own state, which IS this record's to own: zero findings have ever been emitted against the Design Model (Reviewer-lens per-artifact verdicts: Approved at Iters 3, 4, 5 and 6); zero CRs target it (the three Work Order CRs — Architectural Proof-of-Concept, Test Evaluation Summary ×2 — are all DISCHARGED, Issues #9/#11/#12 closed cr:complete, none targeting this artifact); the artifact is converged — 10/10 UC realizations (SEQ-001…SEQ-010), full signatures (CLS-001…CLS-027), interface contracts (INT-006…INT-019) with pre/postconditions, state machines, the four-clause behavioural bar closed end-to-end, both Designer-owned Code Reviewer remediations (F-CR-E3-1 Designer part, F-CR-E3-2) landed and verified against the merged code. NO design content changed this revision; every design section PRESERVED verbatim: §Design Overview, §Domain Model, §Use-Case Realizations, §Design Packages and Classes, §Interface Contracts, §Persistent Data Classes (Database Designer), §Boundary Classes and Navigation Map (User Interface Designer), §Capsules, §Traceability. Optional-artifacts row re-verified this iteration: Data Model trigger NOT fired (oracle re-consulted, fired: false). — Iter 6 (preserved): preservation pass — Status corrected Draft → Approved (the F7 status-honesty lesson applied in the understatement direction); optional-artifacts row re-verified; NO design content changed; the findings-ledger enumeration in that record is superseded by this Iter 7 record. — Iter 5 (preserved): verification pass — the F-CR-E3-2 remediation verified first-hand against the merged code (KeycloakAuthProvider sha 8758844f899f05b6a539ccc84681ee6eb8adf080); state-parameter semantics aligned with the F-CR-E3-3-corrected code (no CSRF claim; round-trip state validation [DEFERRED — lands with the session mechanism, Construction]); verification sha citations updated. — Iter 4 (preserved): record-propagation pass — F-CR-E3-2 resolved (INT-011 extended with BuildAuthorizeRedirectUrl(redirectUri, state) and HandleOidcCallbackAsync(authorizationCode)); F-CR-E3-1 Designer part resolved (INT-016 confirmed unchanged — the interim InMemoryClockingsRepository is a test-seam realization of the final contract). — Iter 3 (preserved): convergence cycle — the Designer-owned part of A-27 landed (four-clause contracts end-to-end: INT-008/010/013 postconditions, CLS-009 note, SEQ-004…007 AF flows, D-9). — Iter 2 (preserved): SEQ-005/006/007 AF-3 flows, D-9, INT-008/010/013. — Iter 1 (preserved): initial Designer contributions. Full per-iteration detail is carried in the Contributions fields below (the authoritative per-iteration records — this changelog summarizes and does not restate them). |
...
| Contributions (Elab Iter 2) | **User Interface Designer** — §Boundary Classes and Navigation Map EVOLVED (not recreated): P-05 extended with the stakeholder-confirmed R001 behavioural bar and its per-UC rendering contracts (UC-004 AF-2 directory cards / UC-005 AF-3 review table — every event row / UC-006 AF-3 CSV — blank cells, no abort / UC-007 AF-3 lookup — still locatable and selectable); **Salt wireframes added** for the two primary screens — SCR-01 Home (first-use affordance, USA-004) and SCR-04 Directory (10-second lookup, USA-003; renders the R001 blank-field contract); §Traceability UI rows extended (SB-05, wireframes, P-05 extension). **Designer** — §Use-Case Realizations EVOLVED (not recreated): SEQ-005/SEQ-006/SEQ-007 extended with the stakeholder-confirmed R001 behavioural bar AF-3 flows (UC-005 AF-3 — every event row rendered, blank display fields, employee not removed, no error; UC-006 AF-3 — every event row written, blank cells, no abort; UC-007 AF-3 — employee locatable and selectable with blank fields); §Design Overview extended with design decision D-9 (GetDisplayData map completeness over the requested uid set) and the Directory-query / Report-export mechanism rows updated to the confirmed bar; §Interface Contracts updated (INT-008 GetDisplayData, INT-010 ILdapGateway, INT-013 IReportExport postconditions); §Traceability Designer rows added; §Document Control updated in place — this update. §Domain Model, §Design Packages and Classes, §Persistent Data Classes (Database Designer), §Boundary Classes and Navigation Map (User Interface Designer), §Capsules — PRESERVED (zero findings on this artifact at the Elaboration Iter 1 LCA review; the bar adds no class and changes no signature). |
| Contributions (Elab Iter 3) | **User Interface Designer** — §Boundary Classes and Navigation Map EVOLVED (not recreated): (1) **P-05 extended to FOUR clauses** per Review Record A-27 (stakeholder contribution at the Iter 2 verdict gate, binding — the fourth clause, verbatim: "a missing attribute is displayed as missing. It is never replaced by a default, placeholder, guessed value, or another employee's value"); the rendering contract formalized as an activity diagram; on-screen em-dash placeholder distinguished from the CSV's truly empty cell (stakeholder rationale, verbatim: "on the CSV that reaches payroll a fabricated department is worse than an empty cell"); (2) **SCR-03 News Salt wireframe added** — the featured-banner stacking contract (stakeholder decision Iter 2, verbatim: "newest first") visualized in the two-featured state, closing the visualization gap on the one UI contract two governance artifacts mis-transcribed in Iter 2 (Development Case F1, Risk List F2); (3) **SCR-04 wireframe contract updated** to cite all four clauses; §Traceability UI rows extended; §Document Control updated in place — this update. **Designer** — §Interface Contracts, §Design Packages and Classes, §Use-Case Realizations, §Design Overview, §Traceability, §Document Control EVOLVED (not recreated) — **the Designer-owned part of A-27 landed this iteration** (the mechanism build now has its four-clause design contract): (1) §Interface Contracts — INT-008 GetDisplayData, INT-010 ILdapGateway, INT-013 IReportExport postconditions extended from three to FOUR clauses (clause d verbatim: a missing attribute is displayed as missing, never replaced by a default, a placeholder, a guessed value, or another employee's value; the value chain closed end-to-end — null is the FINAL mapped value at the gateway, blank the FINAL rendered value on screen, the truly empty cell the FINAL written value in the CSV); (2) §Design Packages and Classes — CLS-009 LdapGateway graceful-degradation note extended to four clauses (no class added, no signature changed); (3) §Use-Case Realizations — SEQ-004/005/006/007 AF flows extended to clauses a/b/c/d, the CSV AF-3 distinguishing the truly empty cell (no placeholder character — the file reaches payroll); (4) §Design Overview — D-9 extended to four clauses, Directory-query and Report-export mechanism rows updated, CLS-009 testability entry extended (substitution-attempt fixtures per A-28 so clause d can actually fail); (5) §Traceability — Designer Iter 3 rows added, the UI Designer's forward-pointer to the CLS-009 contract marked LANDED. §Domain Model, §Persistent Data Classes (Database Designer), §Boundary Classes and Navigation Map (User Interface Designer), §Capsules — PRESERVED (zero findings on this artifact at both LCA reviews; the fourth clause adds no class and changes no signature). |
| Contributions (Elab Iter 4) | **Designer** — §Interface Contracts and §Design Packages and Classes EVOLVED (not recreated), §Traceability extended, §Document Control updated in place — **the two narrative-tracked Code Reviewer findings naming the Designer as remediation owner are RESOLVED this pass** (Review Record Iter 3, Consolidated Finding Tracker rows #7/#8; contract-table hygiene, no behavior change): (1) **F-CR-E3-2** — the INT-011 contract table extended with the two redirect-flow operations the merged R003 mechanism carries (`BuildAuthorizeRedirectUrl(redirectUri, state)` — authorize-URL construction with the state parameter; `HandleOidcCallbackAsync(authorizationCode)` — code→token→JWKS-validation→identity+roles), each with preconditions/postconditions; the `GetAuthenticatedUserAsync(context)` row carrying the merged async signature and its boundary-rejection behavior (row label corrected in this pass — the prior revision annotated the async update but left the label `GetAuthenticatedUser`; verified against the merged code, PR #4); the Infrastructure package diagram synchronized to the same four-operation surface (CLS-010 + internal ValidateTokenAsync seam + CON-004 redirect-flow note — round-trip state validation is Construction scope, the session mechanism; the Elaboration contract generates the state and does not claim its validation); (2) **F-CR-E3-1 (Designer part)** — INT-016 CONFIRMED UNCHANGED: the interim InMemoryClockingsRepository merged for the Elaboration validation (PRs #3/#5) is a test-seam realization of the final contract (same UNIQUE idempotency_key behavior via DuplicateIdempotencyKeyException → RejectedDuplicate, immediate commit, no AddRange), NOT a design change; CLS-012 PgPersistence (EF Core + Npgsql 10.0.3, ADR-002) lands in Construction Iteration 1 (R008 build-time validation). The behavior was inside design all along (CON-004 redirect flow; SEQ-001 middleware behavior); the contract table lagged the code — it no longer does. **User Interface Designer** — §Traceability extended with the storyboard-completion cross-reference rows: **SB-06 (UC-003 Browse News — the BG-003 adoption gateway, carrying the stakeholder-decided featured-banner stacking contract)** and **SB-07 (UC-009 Edit Published News — the audited-edit flow, NFR-005/AUD-002)** were added to the Use-Case Model §Use-Case Specifications → UI Flow References this iteration per the Work Order instruction (storyboards visualizing critical use-case flows for stakeholder validation); this artifact's §Traceability now cross-references them (SCR-01/SCR-03 banner-stack rendering per P-02; SCR-08 → SCR-07 edit-mode navigation per the Navigation Map). **§Boundary Classes and Navigation Map PRESERVED** — zero findings on this artifact across all three LCA reviews; the SB-06/SB-07 flows are realized by the existing screen registry, Navigation Map, and P-02 pattern (no new screen, pattern, or transition emerged — the storyboards visualize flows the boundary classes already realize). No Database Designer contribution this iteration (no finding or CR targets the §Persistent Data Classes section). |
| Contributions (Elab Iter 5) | **Designer** — §Interface Contracts and §Design Packages and Classes EVOLVED (not recreated), §Traceability extended, §Document Control updated in place — **verification pass on the landed F-CR-E3-2 remediation** (the Review Record's Iter 4 snapshot still lists F-CR-E3-2 as open with deadline "next Design Model pass" = this pass; the remediation landed in the artifact's Iter 4 revision after that snapshot was taken): the four-operation INT-011 surface verified first-hand against the current merged code (sha 8758844f899f05b6a539ccc84681ee6eb8adf080) — exact match, no signature drift. Two record-hygiene corrections applied: (1) **state-parameter semantics aligned with the F-CR-E3-3-corrected code** (PR #7, APPROVED) — the INT-011 `BuildAuthorizeRedirectUrl` precondition and the Infrastructure diagram's CLS-010 note no longer label state "the CSRF parameter"; the honest semantics recorded: generated per challenge, carried to the issuer, NO CSRF protection as of Elaboration, round-trip validation [DEFERRED — lands with the session mechanism, Construction]; (2) **verification sha citations updated to the current verified file sha** (the PoC F3 defect class — a citation whose purpose is verifiability must match the file a reviewer reads). SEQ-001 verified clean (carries no state/CSRF claim — no correction owed). No class added; no signature changed; no behavior change. No User Interface Designer or Database Designer contribution this iteration (no finding or CR targets their sections — §Boundary Classes and Navigation Map and §Persistent Data Classes PRESERVED). |
| Contributions (Elab Iter 6) | **Designer** — §Document Control updated in place (preservation pass): the Status field corrected Draft → **Approved** (the R014 record-propagation class — the artifact's own record understated its review state across five review-approved revisions; the F7 status-honesty lesson applied in the understatement direction); the Iter 6 preservation record appended; the optional-artifact oracle re-consulted (Data Model trigger NOT fired — fired: false). NO design section touched: zero findings and zero CRs target the artifact this iteration; every design section PRESERVED verbatim (the artifact is converged — 10/10 UC realizations SEQ-001…SEQ-010, full signatures CLS-001…CLS-027, interface contracts INT-006…INT-019 with pre/postconditions, state machines, the four-clause behavioural bar closed end-to-end, both Designer-owned Code Reviewer remediations landed and verified). No User Interface Designer or Database Designer contribution this iteration (no finding or CR targets their sections — §Boundary Classes and Navigation Map and §Persistent Data Classes PRESERVED). |
| Contributions (Elab Iter 7) | **Designer** — §Document Control updated in place (preservation pass + record-hygiene correction): the R014 post-write staleness subclass corrected in this artifact's own record — the Iter 6 record's findings-ledger enumeration (staled by the same cycle's later events: TES F4/DC F5 ledger-closed 2026-09-03; F-CR-E3-2 closed by the Code Reviewer lens at Iter 6) replaced with a reference to the two owning registries per the stakeholder's Iter 6 verdict-gate directive (the Review Record findings ledger + the SCM issue tracker — live census verified this iteration via scm_list_issues: Issue #14 open as the CR vehicle for the ledger-closed TES F4/A-40, assigned:test-manager, cr:approved; #1/#2/#9/#11/#12 closed cr:complete); the Status field's review-verdict currency extended to Iters 3/4/5/6; the Upstream inputs F-CR-E3-2 closure currency updated; the optional-artifacts row re-verified (Data Model trigger NOT fired at Iter 7 — oracle re-consulted this iteration, fired: false). NO design content changed: zero findings and zero CRs target the Design Model this iteration (verified against the Review Record Iter 6 technical-lens record — Design Model verdict Approved, no new findings; the five Iter 6 findings TES F5/DC F6/UCM F1/SUP F1/TC F2 are owned by Test Manager/Process Engineer/System Analyst/Tester; F-CR-E3-1 is Construction scope per R008); every design section PRESERVED verbatim. No User Interface Designer or Database Designer contribution this iteration (no finding or CR targets their sections — §Boundary Classes and Navigation Map and §Persistent Data Classes PRESERVED). **Verification pass (same iteration, second Designer action — this revision):** the preservation record's observable-state claims were verified against the live sources BEFORE preservation was accepted, per the preservation-pass verification discipline (a record that cites observable state is checked against the live source in the same pass, never accepted on trust — the R014 post-write staleness subclass checked and cleared): scm_list_issues (all states) returns an EXACT MATCH to the recorded census — Issue #14 open (cr:approved, assigned:test-manager — the CR vehicle for the ledger-closed TES F4/A-40), Issues #1/#2/#9/#11/#12 all closed cr:complete — so this record is confirmed accurate and current, not itself a stale census. The optional-artifact oracle re-consulted a second time this iteration (Data Model trigger: fired: false — unchanged). Preservation CONFIRMED: zero findings and zero CRs target the Design Model; no design content changed (every design section PRESERVED verbatim); no stakeholder decision is owed from the Designer this iteration — both [DEFERRED] markers in this artifact (round-trip OIDC state validation — session mechanism, Construction; CLS-012 PgPersistence — R008, Construction Iteration 1) are recorded scope decisions with named owners, not open questions. |
| Upstream inputs | Use-Case Model (UC-001…UC-010, all FULL, UI Flow References incl. SB-01…SB-07 — SB-06/SB-07 added Iter 4; UC-005/006/007 AF-3 flows stakeholder-confirmed; fourth behavioural-bar clause propagated Iter 3 per A-25); Supplementary Specification (SEC-001…007, AUD-001…005, DAT-001/002, USA-001…009, REL-001…004, PRF-001…003, SUP-004, DC-001…010, INT-001…005, STD-001…005; R001 Behavioural Bar — one contract, four consumers); Software Architecture Document (COMP-001…COMP-011, subsystem interfaces, ADR-001…004, timestamp convention: store UTC / display America/Havana / export ISO-8601 with explicit offset / payroll day = local calendar day); Review Record (zero open ledger findings on the Design Model; Iter 3 Consolidated Finding Tracker rows #7/#8 — F-CR-E3-1/F-CR-E3-2, Designer-owned remediations, landed Iter 4, verified Iter 5, and F-CR-E3-2's narrative closure executed by the Code Reviewer lens at Iter 6 [F-CR-E3-1 remains open, Construction scope per R008 — owned by the Implementer, not this artifact]; A-27 fourth-clause propagation — design contracts land with the mechanism build so the code implements four clauses); merged mechanism code (PR #4 KeycloakAuthProvider — introduced the INT-011 four-operation surface; current verified file sha 8758844f899f05b6a539ccc84681ee6eb8adf080, PR #7 comment-only F-CR-E3-3 correction APPROVED [review 5090059324, CI GREEN run 33632200967]; PRs #3/#5 InMemoryClockingsRepository — the INT-016 test-seam realization); stakeholder decisions (Elaboration Iter 2: R001 behavioural bar — behavioural, not statistical, prior >90% criterion dropped as invented; bar applies to all four AD-reading use cases — answer "Yes"; featured banner — "newest first"); stakeholder contribution at the Iter 2 verdict gate (binding, propagated Iter 3: the FOURTH behavioural-bar clause — a missing attribute is displayed as missing, never replaced by a default, a placeholder, a guessed value, or another employee's value) |
| Optional artifacts | Data Model — [OMITTED: trigger not fired per Development Case §5.2 — re-verified via the trigger oracle at Elaboration Iter 7 (fired: false, re-consulted this iteration); data lives inline in this Design Model §Persistent Data Classes]. Architectural Proof-of-Concept — trigger FIRED, owned by the Software Architect (not a Designer artifact). |
## Design Overview
...
**Elaboration Iter 3 evolution (Designer — Review Record A-27, fourth-clause propagation):** design decision D-9 extended from three to FOUR clauses; the Directory-query and Report-export mechanism rows and the CLS-009 testability entry updated to the four-clause bar. No class is added and no signature changes — clause (d) is a postcondition on existing behavior, so the mechanism build (Work Items 7–9) implements four clauses against an unchanged class inventory.

### Design Decisions (Elaboration Iter 1: D-1…D-8; Elaboration Iter 2: D-9; Elaboration Iter 3: D-9 extended — clause d)
...
9. **R001 behavioural bar — one contract, four consumers, FOUR clauses (stakeholder decision, Elaboration Iter 2, bar reach stakeholder-confirmed — answer "Yes"; FOURTH clause added at the Iter 2 verdict gate, propagated Iter 3 per Review Record A-27):** the bar's four clauses — (a) every employee is rendered whether or not their attributes are complete; (b) a missing attribute never removes someone from results; (c) a missing attribute never raises an error; (d) *a missing attribute is displayed as missing. It is never replaced by a default, a placeholder, a guessed value, or another employee's value* (stakeholder contribution, verbatim) — are realized as POSTCONDITIONS on the existing LDAP Query Mechanism classes. No new class, no signature change. The three Iter 2 clauses stop data from being LOST; clause (d) stops it from being INVENTED. The value chain is closed end-to-end: **null** is the FINAL mapped value at the gateway (CLS-009 — INT-010); **blank** is the FINAL rendered value on screen (CLS-017/CLS-020 consumers — INT-008); the **truly empty cell** is the FINAL written value in the CSV (CLS-006 — INT-013; no placeholder character, because the file reaches payroll). CLS-009 LdapGateway maps missing attributes to null and never drops an entry (Search); CLS-003 DirectoryService.GetDisplayData returns a map **complete over the requested uid set** — a uid AD cannot resolve (e.g., a departed employee with clocking history) maps to an all-null `EmployeeDisplayData`, so clause (a) holds mechanically for UC-005/UC-006 without TryGetValue scattering across consumers; CLS-006 ReportExportService writes every event row with truly empty cells for missing display fields (ad_user_id resolves identity — CON-006); CLS-017/CLS-020 render blank display fields without removing the row/entry and without substituting. AF-2 (AD unreachable) remains a distinct condition with a distinct contract (uid-only table for UC-005, abort for UC-006, blocked lookup for UC-007) — the bar does not waive it.
...
| Directory query | CLS-009 LdapGateway: read-only LDAP v3 queries, 5 s hard timeout (PRF-003), graceful degradation — missing attributes are null, the entry is NOT hidden, null NEVER substituted; **R001 behavioural bar, FOUR clauses (stakeholder-confirmed Elab Iter 2; fourth clause propagated Iter 3 — A-27): every employee rendered, no removal, no error, no substitution — one contract, four consumers (UC-004/005/006/007)**; GetDisplayData map complete over the requested uid set (D-9); no local copy (CON-006) | Active Directory over LDAP v3 (CON-005, CON-007) |
| Report export | CLS-006 ReportExportService encapsulates CSV column set v1; month boundaries computed in America/Havana local time; aborts on AD unavailable — no partial file (UC-006 AF-2); missing display attributes → every event row written with TRULY EMPTY cells — no placeholder character, no abort, NEVER a substituted value (UC-006 AF-3 — R001 bar, four clauses; ad_user_id resolves identity; the CSV reaches payroll) | CSV per STD-003 |
...
- **CLS-009 LdapGateway is fakeable** — R001 graceful-degradation tests (missing attributes → blank fields, entry not hidden) run without AD; the bar's FOUR clauses (render / no removal / no error / no substitution) are assertable against a fake ILdapGateway returning deliberately-gapped entries — the same fixture shape the R001 PoC uses against the disposable directory, extended per Review Record A-28 with substitution-attempt fixtures (a default category, a first-office fallback) so clause (d) can actually fail; the real-AD integration test waits on R010 (STK-004 service account).
...
**Elaboration Iter 2 evolution (Designer):** INT-008 GetDisplayData, INT-010, and INT-013 postconditions extended with the R001 behavioural bar (stakeholder decision, Elaboration Iter 2; bar reach stakeholder-confirmed — answer "Yes").

**Elaboration Iter 3 evolution (Designer — Review Record A-27, fourth-clause propagation):** INT-008, INT-010, and INT-013 postconditions extended from three to FOUR clauses. The fourth clause (stakeholder contribution at the Iter 2 verdict gate, binding, verbatim): **"a missing attribute is displayed as missing. It is never replaced by a default, a placeholder, a guessed value, or another employee's value."** The three Iter 2 clauses stop data from being LOST; the fourth stops it from being INVENTED. The contract chain is closed end-to-end: **null** is the FINAL mapped value at the gateway (INT-010); **blank** is the FINAL rendered value on screen (INT-008 consumers); the **truly empty cell** is the FINAL written value in the CSV (INT-013) — no consumer anywhere in the chain may substitute. All other rows preserved exactly as reviewed.

**Elaboration Iter 4 evolution (Designer — Review Record Iter 3, Consolidated Finding Tracker rows #7/#8; contract-table hygiene, no behavior change):** (1) **F-CR-E3-2 RESOLVED** — INT-011 extended with the two redirect-flow operations the merged R003 mechanism carries (PR #4, `KeycloakAuthProvider` — file sha 7bd4cfd at merge; current verified file sha 8758844f, PR #7 comment-only): `BuildAuthorizeRedirectUrl(redirectUri, state)` and `HandleOidcCallbackAsync(authorizationCode)`, each with preconditions/postconditions; the `GetAuthenticatedUserAsync(context)` row carrying the merged async signature and its boundary-rejection behavior (row label corrected in this pass — the prior revision annotated the async update but left the label `GetAuthenticatedUser`; verified against the merged code — PR #4 introduced the surface, current verified file sha 8758844f). The behavior was inside design all along (CON-004 redirect flow; SEQ-001 middleware behavior); the contract table lagged the code — it no longer does. (2) **F-CR-E3-1 (Designer part) RESOLVED** — INT-016 CONFIRMED UNCHANGED: the interim `InMemoryClockingsRepository` merged for the Elaboration validation (PRs #3/#5) is a **test-seam realization** of this contract — same UNIQUE idempotency_key behavior (REL-002), immediate commit, no `AddRange` — NOT a design change; CLS-012 PgPersistence (EF Core + Npgsql 10.0.3, ADR-002) lands in Construction Iteration 1 (R008 build-time validation). All other rows preserved exactly as reviewed.

**Elaboration Iter 5 evolution (Designer — verification pass; F-CR-E3-2 closure verification + F-CR-E3-3 semantics alignment):** the landed Iter 4 INT-011 extension is verified first-hand against the current merged code (`KeycloakAuthProvider` sha 8758844f899f05b6a539ccc84681ee6eb8adf080 — PR #4 introduced the four-operation surface; PR #7's APPROVED comment-only F-CR-E3-3 correction [review 5090059324, CI GREEN run 33632200967] left the interface surface unchanged): all four operations match exactly (`ConfigureOidc`, `BuildAuthorizeRedirectUrl`, `HandleOidcCallbackAsync`, `GetAuthenticatedUserAsync`). Two corrections applied, both record-hygiene class: (1) **state-parameter semantics aligned with the F-CR-E3-3-corrected code** — the `BuildAuthorizeRedirectUrl` precondition no longer labels state "the CSRF parameter": per the corrected code (PR #7), the parameter is generated per challenge and carried to the issuer but provides NO CSRF protection as of Elaboration — round-trip state validation is [DEFERRED — lands with the session mechanism, Construction]; (2) **verification sha citations updated to the current verified file sha** (the PoC F3 defect class, applied preemptively — a citation whose purpose is verifiability must match the file a reviewer reads). No signature changed; no behavior changed; all other rows preserved exactly as reviewed.
...
| INT-008 | IDirectoryService | GetDisplayData(uids) | caller authenticated | Map uid → EmployeeDisplayData (name, department, office) **COMPLETE over the requested uid set (design decision D-9)**: every requested uid has a map entry — a uid AD cannot resolve maps to an all-null EmployeeDisplayData; missing attributes null within resolved entries — **R001 behavioural bar, FOUR clauses (stakeholder-confirmed Elab Iter 2; fourth clause added at the Iter 2 verdict gate, propagated Iter 3 — A-27): (a) every employee rendered, (b) no removal from results, (c) no error raised, (d) a missing attribute is displayed as missing — never replaced by a default, a placeholder, a guessed value, or another employee's value**; throws on AD unavailable |
...
| INT-010 | ILdapGateway | Search(criteria) / GetDisplayData(uids) | connection configured (R010 service account) | Read-only LDAP v3 (CON-007); 5 s hard timeout (PRF-003); missing attributes mapped null, entry NOT dropped, no error raised — **R001 behavioural bar, FOUR clauses (stakeholder-confirmed Elab Iter 2; fourth clause propagated Iter 3 — A-27): one contract, four consumers (UC-004/005/006/007); null is the FINAL mapped value — never substituted by a default, a placeholder, a guessed value, or another employee's value**; GetDisplayData map complete over the requested uid set (D-9) |
| INT-011 | IAuthProvider | ConfigureOidc(builder, options) | Keycloak client settings present in configuration (validated — `options.Validate()`) | OIDC middleware registered at the request boundary; all pages require authentication (SEC-003); roles mapped from claims (SEC-002) |
| INT-011 | IAuthProvider | BuildAuthorizeRedirectUrl(redirectUri, state) — **added Iter 4 (F-CR-E3-2); precondition corrected Iter 5 (F-CR-E3-3 semantics)** | redirectUri non-empty; state non-empty (generated per challenge by the middleware and carried to the issuer — provides NO CSRF protection as of Elaboration: round-trip state validation is [DEFERRED — lands with the session mechanism, Construction] — F-CR-E3-3-corrected semantics, PR #7) | Returns the issuer's authorize URL (client_id, redirect_uri, response_type=code, state) — the portal's half of the CON-004 redirect flow (SEQ-001, at the middleware); the Elaboration contract generates the state and does not claim its validation |
| INT-011 | IAuthProvider | HandleOidcCallbackAsync(authorizationCode) — **added Iter 4 (F-CR-E3-2)** | authorizationCode non-empty (the issuer's callback carries the code) | code → token → JWKS validation (RS256 signature with kid match; exp/iss/aud/sub enforced) → AuthenticatedUser (Uid + Roles verbatim from claims, SEC-006); every rejection is OidcTokenValidationException → 401 at the request boundary |
| INT-011 | IAuthProvider | GetAuthenticatedUserAsync(context) — **row updated Iter 4 to the merged async signature; label corrected this pass (was `GetAuthenticatedUser`)** | — | AuthenticatedUser (Uid + Roles) or null if unauthenticated; throws OidcTokenValidationException when the presented token is expired or invalid — rejected at the boundary (401), never passed downstream |
...
| INT-013 | IReportExport | ExportMonth(year, month) | caller holds HR Administrator role | Month boundaries computed as local calendar days in America/Havana (payroll day = local day); CSV column set v1 (ad_user_id, employee_name, department, office, event_timestamp ISO-8601 with explicit offset, event_type); **missing display attributes → EVERY event row written with TRULY EMPTY cells — no placeholder character, no abort, NEVER a substituted value (UC-006 AF-3 — R001 bar, FOUR clauses; clause d: a missing attribute is displayed as missing, never replaced by a default, a placeholder, a guessed value, or another employee's value; ad_user_id resolves identity — the CSV reaches payroll: an empty cell gets questioned, a plausible wrong one does not)**; ExportAborted.DirectoryUnavailable if AD unreachable — NO partial file (AF-2); NoData if no events (AF-1) |
...
| INT-016 | IClockingsRepository | Add / AddRange / GetByEmployeeAndRange / GetByFilter / GetByRange | — | Stages / queries; UNIQUE idempotency_key enforced at the database (REL-002); returned events are immutable. **F-CR-E3-1 confirmation (Iter 4): INT-016 is UNCHANGED** — the interim InMemoryClockingsRepository merged for the Elaboration validation (PRs #3/#5) is a test-seam realization of this contract (same UNIQUE-key behavior via DuplicateIdempotencyKeyException → RejectedDuplicate, immediate commit, no AddRange), not a design change; CLS-012 PgPersistence (EF Core + Npgsql 10.0.3, ADR-002) lands in Construction Iteration 1 (R008 build-time validation) |
...
### Elaboration Iteration 6 — Verification Record (record-propagation, same-pass discipline)

**Status: schema baseline V1 STABLE — re-verified against the observed state this iteration; no schema evolution warranted; the section's clean-review count extends to FIVE completed LCA reviews (Iters 1–5).** This record propagates the observed state into the section that owns the schema, per the same-pass record-propagation discipline. What was verified, first-hand, this iteration (2026-09-03):

- **Trigger re-check:** the Development Case oracle reports the Data Model optional trigger **NOT FIRED** (5 persistent entities, no data migration in scope, AD data never copied — CON-006). This section remains the data model's home; the standalone Data Model artifact is declared [OMITTED: Data Model — trigger not fired].
- **Constraint guard:** the SAD's declared engine (PostgreSQL, CON-003 / ADR-002 / Npgsql 10.0.3, COMP-008 PG Persistence) matches the stakeholder's declared constraint — no mismatch; all DDL above targets PostgreSQL syntax.
- **Formal trace links re-verified first-hand:** CLS-021→`clockings` and CLS-024→`worker_categories` re-queried via the trace model this iteration — both intact with correct descriptions (uk_clockings_idempotency_key / REL-002; timestamptz UTC / DAT-001; natural PK employee_uid, two data columns only / CON-006; no CHECK on category / ADR-004). The remaining three Realizes links (CLS-022→`news_items`, CLS-023→`news_audit`, CLS-025→`category_audit`) are carried from the Iter 5 re-query — the class inventory is unchanged, nothing owed.
- **Findings/CR sweep (first-hand; census refreshed at the Iter 7 pass):** zero findings from any review lens target this section or the Design Model — **clean at all FIVE completed LCA reviews (Iters 1–5)**; the Iter 5 technical-lens per-artifact verdict is Design Model **Approved** (INT-011's four-operation surface verified against the merged code; F-CR-E3-2's Designer-owned remediation landed; zero new findings). At this record's write the open SCM CRs were #11 (PoC sha citation, Software Architect) and #12 (TES remainder-enumerations, Test Manager) — both targeting other artifacts; both transitioned **cr:complete** later in the same cycle (post-write staleness, R014 subclass — the census is refreshed here per the same-pass discipline; live census at the Iter 7 refresh: **1 open** — Issue #14, TES F4/A-40, assigned:test-manager, targets the Test Evaluation Summary, not this artifact; **5 closed** — #1/#2/#9/#11/#12).
- **No R014-class staleness:** this section enumerates no remaining work that has since landed — the Construction hand-off it names (the PG adapter, R008 / F-CR-E3-1, `[DEFERRED]` marker carried) remains open Construction scope, accurately marked.
- **No open consequential doubt in the persistence domain:** every decision traces to a recorded stakeholder answer or a declared constraint — engine (CON-003); timestamp convention (stakeholder, Elab Iter 1: store UTC, display office-local, ISO-8601 offset export, payroll day = local calendar day); office timezone (stakeholder: America/Havana); fourth behavioural clause (stakeholder, Iter 2 verdict gate — display-rendering only, persistence-neutral since AD attributes are never stored, CON-006); featured-banner stack (stakeholder: newest first — served by the existing `is_featured` column plus `ix_news_items_published` ordering, a query concern); offline queue (stakeholder: Yes — browser localStorage, client-side, never a table); category list (CON-013 + ADR-004: JSON file, no schema).
- **Construction hand-off unchanged:** the O/R mapping table above remains the implementation contract for the Construction Iteration 1 PostgreSQL adapter (R008) — identity, value converters, conflict clauses, partial indexes, audit REVOKEs.

**Traceability (Iteration 6 verification record):**

| Element | Traces From | Link Type | Traces To |
|---|---|---|---|
| Iter 6 verification record (this subsection) | Development Case oracle (Data Model trigger NOT FIRED, re-checked first-hand 2026-09-03); SAD §Logical View COMP-008 + ADR-002 (engine match — constraint guard); trace model re-query (CLS-021 / CLS-024 Realizes links verified first-hand; CLS-022/023/025 carried from the Iter 5 re-query); scm_list_issues open sweep (Issues #11/#12 — neither targets this artifact); Review Record Iter 5 technical-lens verdict (Design Model Approved — fifth consecutive clean LCA review) | Refines | Construction Iteration 1 PG adapter (R008 / F-CR-E3-1 — the O/R mapping table is its contract); Migration V1 (stable core — Construction adds tables, never restructures) |

### Elaboration Iteration 7 — Verification Record (record-propagation, same-pass discipline)

**Status: schema baseline V1 STABLE — re-verified against the observed state this iteration; no schema evolution warranted; the section's clean-review count extends to SIX completed LCA reviews (Iters 1–6).** This record propagates the observed state into the section that owns the schema, per the same-pass record-propagation discipline. What was verified, first-hand, this iteration (2026-09-03):

- **Trigger re-check:** the Development Case oracle reports the Data Model optional trigger **NOT FIRED** (5 persistent entities, no data migration in scope, AD data never copied — CON-006). This section remains the data model's home; the standalone Data Model artifact is declared [OMITTED: Data Model — trigger not fired].
- **Constraint guard:** the SAD's declared engine (PostgreSQL, CON-003 / ADR-002 / Npgsql 10.0.3, COMP-008 PG Persistence) matches the stakeholder's declared constraint — no mismatch; all DDL above targets PostgreSQL syntax.
- **R014 post-write staleness remediated in-section (the driver of this iteration's touch):** the Iter 6 verification record's census bullet named Issues #11/#12 as the open SCM CRs — accurate at its write time, staled later in that same cycle when both transitioned cr:complete. The bullet is refreshed in place above (live census: 1 open — Issue #14, TES F4/A-40, targets the Test Evaluation Summary, not this artifact; 5 closed). This is the same census-currency correction class the Iter 6 review minted as TES F5 / DC F6 / UCM F1 / SUP F1 / TC F2 against the other artifacts (actions A-42…A-46); applying it here pre-empts the successor finding the Iter 7 review would otherwise mint against this section.
- **Formal trace links — rotation continues:** CLS-022→`news_items` re-queried via the trace model this iteration — intact, correct description (soft delete via status CHECK, CON-012, no delete path). With CLS-021/CLS-024 verified at Iter 6 and CLS-023/CLS-025 at Iter 5, all five Realizes links are first-hand-verified within a three-iteration rotation window; the class inventory is unchanged, nothing owed.
- **Class inventory unchanged:** no new persistent classes entered the tree since the Iter 6 verification — the E6 tree growth was documentation only (per the Code Reviewer lens's same-day Iter 6 verification, recorded in the Review Record); the mechanism code stores nothing (CLS-009 LdapGateway reads AD live — CON-006; CLS-008 OfflineQueueClient queues in browser localStorage; CLS-010 KeycloakAuthProvider is stateless). The five-table mapping above is complete and correct.
- **Diagrams re-validated:** both embedded diagrams (Physical Schema; Index Strategy) re-validated via the UML validator this iteration — both OK; no diagram content changed.
- **Findings/CR sweep (first-hand):** zero findings from any review lens target this section or the Design Model — **clean at all SIX completed LCA reviews (Iters 1–6)**; the Iter 6 technical-lens per-artifact verdict is Design Model **Approved** (INT-011's four-operation surface verified against the merged code sha 8758844f; F-CR-E3-2 closed by the Code Reviewer lens at Iter 6; co-owned sections preserved; zero new findings). The five open Iter 6 findings (TES F5, DC F6, UCM F1, SUP F1, TC F2 — actions A-42…A-46) are census/remainder refreshes owned by the Test Manager, Process Engineer, System Analyst, and Tester; none targets this artifact, and none requires a schema change. F-CR-E3-1 (interim in-memory ClockingsRepository vs INT-016) remains open Construction scope with its `[DEFERRED]` marker — accurately carried.
- **No open consequential doubt in the persistence domain:** every decision traces to a recorded stakeholder answer or a declared constraint — engine (CON-003); timestamp convention (stakeholder, Elab Iter 1: store UTC, display office-local, ISO-8601 offset export, payroll day = local calendar day); office timezone (stakeholder: America/Havana); fourth behavioural clause (stakeholder, Iter 2 verdict gate — display-rendering only, persistence-neutral since AD attributes are never stored, CON-006); featured-banner stack (stakeholder: newest first — served by the existing `is_featured` column plus `ix_news_items_published` ordering, a query concern); offline queue (stakeholder: Yes — browser localStorage, client-side, never a table); category list (CON-013 + ADR-004: JSON file, no schema).
- **Construction hand-off unchanged:** the O/R mapping table above remains the implementation contract for the Construction Iteration 1 PostgreSQL adapter (R008) — identity, value converters, conflict clauses, partial indexes, audit REVOKEs.

**Traceability (Iteration 7 verification record):**

| Element | Traces From | Link Type | Traces To |
|---|---|---|---|
| Iter 7 verification record (this subsection) | Development Case oracle (Data Model trigger NOT FIRED, re-checked first-hand 2026-09-03); SAD §Logical View COMP-008 + ADR-002 (engine match — constraint guard); trace model re-query (CLS-022 Realizes link verified first-hand; CLS-021/024 verified Iter 6, CLS-023/025 carried from Iter 5); scm_list_issues live census (1 open — Issue #14, targets the TES, not this artifact; 5 closed #1/#2/#9/#11/#12); Review Record Iter 6 technical-lens verdict (Design Model Approved — sixth consecutive clean LCA review); Iter 6 findings ledger (A-42…A-46 — none targets this artifact) | Refines | Construction Iteration 1 PG adapter (R008 / F-CR-E3-1 — the O/R mapping table is its contract); Migration V1 (stable core — Construction adds tables, never restructures) |
...
| CLS-003 DirectoryService | UC-004 (ACL-016), FR-010, R001 + R001 behavioural bar (stakeholder-confirmed, Elab Iter 2 — GetDisplayData map completeness, D-9; four clauses per A-27, Iter 3) | Derives | INT-008; CLS-009 |
...
| CLS-006 ReportExportService | UC-006 (ACL-019), FR-002, INT-005, STD-003, UC-006 AF-3 (R001 behavioural bar, stakeholder-confirmed Elab Iter 2 — truly empty cells, every row written, never substituted per A-27, Iter 3) | Derives | INT-013; CLS-003, CLS-007, CLS-012 |
...
| CLS-009 LdapGateway | CON-005, CON-006, CON-007, R001 + R001 behavioural bar (stakeholder-confirmed, Elab Iter 2 — one contract, four consumers; FOUR clauses per A-27, Iter 3 — null is the FINAL mapped value, never substituted), PRF-003 | Derives | INT-010; Active Directory (external) |
...
| CLS-017 ClockingController | UC-001, UC-002, UC-005, UC-006 (ACL-010), UC-005 AF-3 (R001 bar — renders every event row with blank display fields, never substituted) | Derives | INT-006, INT-013, INT-014 |
...
| CLS-020 CategoryController | UC-007 (ACL-013), UC-007 AF-3 (R001 bar — renders employee locatable/selectable with blank fields, never substituted) | Derives | INT-009, INT-008 |
...
| SEQ-004 | UC-004 (FR-010) + UC-004 AF-2 (R001 behavioural bar, FOUR clauses per A-27, Iter 3 — blank fields, entry not hidden, never substituted) | Realizes | CLS-019, CLS-003, CLS-009 |
| SEQ-005 | UC-005 (FR-001) + UC-005 AF-3 (R001 behavioural bar, stakeholder-confirmed Elab Iter 2; FOUR clauses per A-27, Iter 3 — every event row rendered, blank display fields, no removal, no error, no substitution) | Realizes | CLS-017, CLS-001, CLS-003, CLS-009, CLS-011 |
| SEQ-006 | UC-006 (FR-002) + UC-006 AF-3 (R001 behavioural bar, stakeholder-confirmed Elab Iter 2; FOUR clauses per A-27, Iter 3 — every event row written, truly empty cells, no abort, never substituted) | Realizes | CLS-017, CLS-006, CLS-007, CLS-003, CLS-009, CLS-011 |
| SEQ-007 | UC-007 (FR-003) + UC-007 AF-3 (R001 behavioural bar, stakeholder-confirmed Elab Iter 2; FOUR clauses per A-27, Iter 3 — employee locatable and selectable with blank fields, never substituted) | Realizes | CLS-020, CLS-004, CLS-003, CLS-009, CLS-005, CLS-011 |
...
| **Designer evolution (Elaboration Iter 3 — fourth-clause propagation, Review Record A-27)** | | | |
| D-9 extension — clause (d) | R001 behavioural bar FOURTH clause (stakeholder contribution at the Iter 2 verdict gate, binding, verbatim: "a missing attribute is displayed as missing. It is never replaced by a default, a placeholder, a guessed value, or another employee's value") + Review Record A-27 (design contracts land with the mechanism build so the code implements four clauses) | Derives | INT-008, INT-010, INT-013 (four-clause postconditions); CLS-009, CLS-003, CLS-006, CLS-017, CLS-020; R001 PoC (Architect — FOUR-clause × four-consumer evidence, TC-011 + TC-021/022/023 per A-29); P-05 rendering-contract activity diagram (UI Designer, Iter 3) |
| INT-008 / INT-010 / INT-013 four-clause postconditions | UC-004 AF-2, UC-005 AF-3, UC-006 AF-3, UC-007 AF-3 + fourth clause (verbatim, Iter 2 verdict gate) + A-27; value chain closed end-to-end: null FINAL at the gateway (INT-010), blank FINAL on screen (INT-008 consumers), truly empty cell FINAL in the CSV (INT-013) | Refines | CLS-003, CLS-009, CLS-006; Implementer mechanism build (Work Item 7 — R001); Test Case TC-011 + TC-021/022/023 (fourth-clause verification steps per A-28) |
| CLS-009 graceful-degradation contract (four clauses) | CON-005/006/007, R001 + fourth clause (A-27) — MapEntry leaves missing attributes null as the FINAL value; no default, no placeholder, no guessed value, no another-employee's-value | Refines | INT-010; R001 PoC (disposable LDAP directory, gaps seeded deliberately + substitution-attempt fixtures per A-28); TC-011 |
| SEQ-004/005/006/007 AF-flow four-clause extension | UC-004 AF-2, UC-005 AF-3, UC-006 AF-3, UC-007 AF-3 + fourth clause (A-27); CSV AF-3 writes truly empty cells (no placeholder character — the file reaches payroll) | Realizes | CLS-009, CLS-003, CLS-006, CLS-017, CLS-020; TC-021/022/023 (four-consumer bar evidence); SB-02 frame 4, SB-05 (Use-Case Model) |
| **Designer evolution (Elaboration Iter 4 — contract-table hygiene, Review Record Iter 3 Consolidated Finding Tracker rows #7/#8)** | | | |
| INT-011 contract-table extension — BuildAuthorizeRedirectUrl(redirectUri, state) + HandleOidcCallbackAsync(authorizationCode) | CON-004 (redirect flow: register a client, redirect for login, validate the token, read roles from claims — nothing more); SEQ-001 (middleware behavior); SEC-002/SEC-003/SEC-006; R003; merged PR #4 (KeycloakAuthProvider — introduced the four-operation surface); Review Record F-CR-E3-2 (Code Reviewer lens, Iter 3 — the behavior is inside design; the contract table lagged the code) | Refines | CLS-010 KeycloakAuthProvider (four-operation surface, §Design Packages and Classes Infrastructure diagram — synchronized this pass); OidcMiddleware (request-boundary enforcement — 401 on every rejection); Construction session mechanism (round-trip state validation — deferred, NOT claimed by the Elaboration contract) |
| INT-016 confirmation — contract UNCHANGED (F-CR-E3-1, Designer part) | INT-016 final contract (Add/AddRange staging, commit via INT-015 SaveChanges); REL-002 (UNIQUE idempotency_key — the enforcement point); ADR-002 (EF Core + Npgsql 10.0.3); R008 (Construction build-time validation); merged PRs #3/#5 (InMemoryClockingsRepository — test-seam realization: same UNIQUE-key behavior via DuplicateIdempotencyKeyException → RejectedDuplicate, immediate commit, no AddRange); Review Record F-CR-E3-1 (Code Reviewer lens, Iter 3 — "Designer confirms INT-016 unchanged") | Refines | CLS-012 ClockingsRepository (PgPersistence — Construction Iteration 1); INT-015 IPersistence; uk_clockings_idempotency_key (Database Designer, §Persistent Data Classes) |
| **Designer evolution (Elaboration Iter 5 — F-CR-E3-2 closure verification + F-CR-E3-3 semantics alignment)** | | | |
| INT-011 verification pass — four-operation surface confirmed against the merged code | Review Record F-CR-E3-2 (Code Reviewer lens, Iter 3 — remediation owner: Designer, deadline "next Design Model pass" = this pass; the remediation landed in the artifact's Iter 4 revision, after the Review Record's Iter 4 snapshot); merged PR #4 (introduced the surface) + current verified file sha 8758844f899f05b6a539ccc84681ee6eb8adf080 (PR #7 comment-only F-CR-E3-3 correction APPROVED — review 5090059324, CI GREEN run 33632200967 — left the interface surface unchanged); CON-004; SEQ-001 | Verifies | CLS-010 KeycloakAuthProvider (§Design Packages and Classes Infrastructure diagram); INT-011 contract rows (§Interface Contracts); Construction session mechanism (round-trip state validation — deferred, NOT claimed by the Elaboration contract) |
| INT-011 state-parameter semantics correction (BuildAuthorizeRedirectUrl precondition + CLS-010 diagram note) | PR #7 (F-CR-E3-3 remediation, APPROVED — the code's honest semantics: state generated per challenge and carried to the issuer, NO CSRF protection as of Elaboration; round-trip validation [DEFERRED — lands with the session mechanism, Construction]); the PoC F3 defect class (citation vs verified state — applied preemptively to the sha citations: PR #4 sha 7bd4cfd at merge → current verified file sha 8758844f) | Refines | CLS-010 KeycloakAuthProvider; OidcMiddleware (request-boundary enforcement); Construction session mechanism (owns round-trip state validation when it lands); R003 acceptance criteria (token validation — unaffected by state semantics) |
...
| Design-reference reconciliations 1–5 | CON-011 (verified against repository source sha ba1cb26e), CON-007, CON-012, UC-006 step 1, Use-Case Model (UC-ID authority), CON-006 (reconciliation 4 — no sync mechanism), CON-005 (reconciliation 5 — filter options from AD on demand) | Mitigates | R007 (UI fidelity risk); Implementer (no sync job, no hardcoded filter lists); SCR-04 footer + filter selects |
...
| P-05 R001-bar extension (all four AD-reading UCs) | UC-004 AF-2, UC-005 AF-3, UC-006 AF-3, UC-007 AF-3 (stakeholder-confirmed, Elaboration Iter 2) + FOURTH clause (stakeholder contribution at the Iter 2 verdict gate, propagated Iter 3 — A-27: a missing attribute is displayed as missing, never replaced by a default, a placeholder, a guessed value, or another employee's value) | Refines | Designer view classes (blank-value rendering); Implementer screens; SB-05 (Use-Case Model); R001 PoC (Architect — FOUR-clause × four-consumer evidence, TC-011 + TC-021/022/023) |
| Featured-banner rendering contract (P-02) | FR-006, FR-007, CON-011 (single-banner mockup) + stakeholder decision (Elaboration Iter 2, recorded verbatim: "newest first" — stack ALL featured banners, ordered newest first; every featured flag keeps its banner promise, none silently dropped) | Refines | SCR-01, SCR-03 (banner stack rendering); UC-003 step 4, UC-008 step 3 (Use-Case Model); Implementer screen construction; SCR-03 Salt wireframe + SB-03 frame 4 (visualization added Iter 3) |
| Reconciliations 4–5 (design-reference verification, this revision) | CON-011 (authoritative source read from repository, sha ba1cb26e), CON-006 (reconciliation 4 — the reference's directory-footer sync claim is dropped; no sync job exists), CON-005 (reconciliation 5 — filter select options populate from AD on demand; the reference's hardcoded office/department values are sample data, not specification) | Mitigates | R007 (UI fidelity risk); SCR-04 (footer text + filter selects); Implementer (must not build a sync job or hardcode filter lists) |
| SCR-01 wireframe live-clock element | CON-011 (reference `.now` element — 40 px tabular numerals beside the clocking button, verified in repository source sha ba1cb26e), USA-008 (renders current America/Havana local time; presentational only — never a data field, never a substitute for the recorded timestamp) | Derives | HomeView; Implementer screen construction; SB-01 (Use-Case Model) |
| **UI evolution (User Interface Designer — Elaboration Iter 3 — fourth-clause propagation A-27 + featured-banner stack visualization)** | | | |
| P-05 four-clause rendering-contract activity diagram | R001 behavioural bar FOUR clauses (stakeholder decision Elab Iter 2 + stakeholder contribution at the Iter 2 verdict gate, verbatim: "a missing attribute is displayed as missing. It is never replaced by a default, a placeholder, a guessed value, or another employee's value") + Review Record A-27 (design contracts land with the mechanism build so the code implements four clauses) | Refines | CLS-009 graceful-degradation contract (Designer-owned part of A-27, §Interface Contracts INT-010 — **LANDED Iter 3, this evolution**); Implementer screens; Test Case TC-011 + TC-021/022/023 (fourth-clause verification steps per A-28) |
| SCR-03 News Salt wireframe (featured-banner stack) | FR-006, FR-007, CON-011 (warn-tinted banner style, verified sha ba1cb26e) + stakeholder decision (Elaboration Iter 2, verbatim: "newest first" — stack ALL featured banners, ordered newest first) | Derives | NewsView; Implementer screen construction; SB-03 frame 4 + UC-003 compact reference (Use-Case Model, updated Iter 3); UC-003 step 4, UC-008 step 3 |
| **UI evolution (User Interface Designer — Elaboration Iter 4 — storyboard completion cross-references)** | | | |
| SB-06 storyboard (UC-003 Browse News — Use-Case Model, added Iter 4) | FR-007, BG-003 (80% adoption within 3 months), R002 (adoption risk), CON-011 + stakeholder decision (Elaboration Iter 2, verbatim: "newest first" — stack ALL featured banners, ordered newest first) | Refines | SCR-01, SCR-03 (banner stack rendering — P-02); SCR-03 Salt wireframe (this artifact); SB-03 frame 4 (publish-side banner contract); stakeholder validation (STK-001, STK-003) |
| SB-07 storyboard (UC-009 Edit Published News — Use-Case Model, added Iter 4) | FR-008, NFR-005, AUD-002 (all versions traceable), CON-012, SEC-006 | Refines | SCR-08 → SCR-07 (edit mode) navigation (Navigation Map); SB-03 (publish) + SB-04 (unpublish) — the news-lifecycle storyboard set; stakeholder validation (STK-001) |
