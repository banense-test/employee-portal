...
|---|---|
| Phase | Elaboration |
| Status | Draft — Reviewer verdict at the Elaboration Iter 2 LCA re-review: **Approved, zero findings** (FURPS+ quantified and testable; the R001 behavioural bar recorded as one reliability contract with four consumers + distinct-condition boundary vs AD-unreachable; the >90% figure verified ABSENT; all thresholds tagged with named bases; no gold-plating). This revision (Iter 3) propagates the stakeholder's FOURTH behavioural-bar clause (A-26) — no other content changes. **Iter 4 (preserved):** no finding or CR targets this artifact — no requirement, threshold value, or contract changed; the ONE content delta is record propagation by the Requirements Specifier (threshold owner per the recorded stakeholder decision): the REL-002/REL-003 quantified-threshold rows in §Reliability now carry their OBSERVED validation status (R004 drop simulation, Elaboration Iter 3, CI run 33617748483) so the normative table does not lag the delivery its contracts specify; the `[ASSUMPTION]` derivation-basis tags remain in place; Document Control milestone record updated to the observed validation state (record propagation only). **Iter 6 (preserved):** no finding or CR targets this artifact — zero findings across all five LCA review cycles (Iter 5 technical-lens verdict: PRESERVED — clean at all four prior LCA reviews, unchanged this pass); no requirement, threshold value, or contract changed; the ONE content delta is the same-pass record-propagation correction (R014 mitigation): the Document Control milestone record's remainder-enumeration updated to the observed state; all FURPS+ sections, the four-clause R001 contract, and the REL-002/REL-003 observed-status rows preserved verbatim. **Iter 7 (this revision — R014 record-correction cycle, cycle 2):** SUP F1 (Minor, Reviewer lens, Iter 6 — post-write staleness subclass, confined to this Document Control record) RESOLVED via A-45: stale facts corrected (A-40/A-41 landed and ledger-closed 2026-09-03; live SCM census 1 open CR vehicle, not zero; zero-Critical held four consecutive cycles) and the remainder-enumeration replaced by an owner-reference per the stakeholder's Iter 6 convergence directive; NO requirement, threshold value, or contract changed — all FURPS+ sections, the four-clause R001 contract, and the REL-002/REL-003 observed-status rows preserved verbatim; specification content clean at all six LCA review cycles |
| Milestone Target | End of Elaboration (LCA) — NOT yet achieved; sanction still withheld on the stakeholder's all-findings directive. The empirical validation this specification's contracts demand is OBSERVED: formal TC-001…TC-023 pass complete — 15 PASS · 0 FAIL · 8 BLOCKED (the 8 BLOCKED are a recorded SCOPE decision, deferred to Construction, not missing — stakeholder framing directive, Iter 3); the four-clause R001 behavioural bar (this artifact's Reliability contract) PASS across all four consumers (TC-011 + TC-021/022/023, clause (d) verified against substitution-attempt fixtures); R003 token-validation matrix PASS; R004 drop simulation PASS (REL-002/REL-003 thresholds exercised: zero duplicates, zero losses, sync ≤ 60 s, < 1 s confirmations); execution trace CI run 33617748483; mechanisms merged (PRs #3/#4/#5 to iteration/E1 APPROVED ×3; PR #6 baseline-close to main APPROVED); Issue #1 closed. **Iter 7 correction (SUP F1 / A-45 — post-write staleness subclass):** A-40 (TES F4, Major) and A-41 (DC F5, Minor) landed and ledger-closed 2026-09-03 — the prior enumeration named both as remaining, stale; live SCM census at this revision: **1 open CR vehicle** (Issue #14 — cr:approved, assigned:test-manager; the CR vehicle for the ledger-closed TES F4/A-40, awaiting its cr:complete transition) and **5 closed** (Issues #1/#2/#9/#11/#12, all cr:complete) — the prior record claimed zero open, stale; zero Critical findings open (held four consecutive cycles). **Remaining pre-R6 work is not enumerated in this artifact:** per the stakeholder's Iter 6 directive (verbatim: "State the remaining work ONCE, in a single section that owns it. Every other place that needs it — Milestone Target, the master-workflow diagram, the schedule — references that section instead of restating it"), that state has a single owner — the Review Record findings ledger and the SCM issue tracker — and this record references that owner rather than restating its contents, because a document that copies another artifact's state drifts from it forever. The requirements baseline (FURPS+ contracts) carries zero open findings across all six LCA review cycles |
| Iteration | **Iter 7 (this revision — R014 record-correction cycle, cycle 2; System Analyst):** SUP F1 (Minor, Reviewer lens, Iter 6) resolved via A-45 — same-pass record correction confined to this Document Control record: stale facts corrected (A-40/A-41 landed and ledger-closed 2026-09-03; live census 1 open CR vehicle Issue #14, 5 closed; zero-Critical held four cycles) and the remainder-enumeration REPLACED by an owner-reference (Review Record findings ledger + SCM issues) per the stakeholder's Iter 6 convergence directive; NO requirement, threshold value, or contract changed; the REL-002/REL-003 observed-status rows, the `[ASSUMPTION]` derivation-basis tags, and all FURPS+ sections preserved verbatim; remainder re-verified against the verified findings ledger and the live SCM census immediately before upsert. **Iter 6 (preserved — R014 record-correction cycle; Requirements Specifier):** same-pass record-propagation correction ONLY (R014 mitigation — the DC's binding same-pass discipline): the Document Control milestone record's remainder-enumeration updated to the observed state — it still named the Iter 4 remainder (PoC results ledger A-32 + four Minor record corrections + PM close-pass), every item of which landed and ledger-closed at Iter 4, while A-37/A-38/A-39 landed and ledger-closed at Iter 5 (the two Work Order CRs discharged). No requirement, threshold value, or contract changed; the REL-002/REL-003 observed-status rows, the `[ASSUMPTION]` derivation-basis tags, and all FURPS+ sections preserved verbatim; zero findings and zero CRs target this artifact (clean at all five LCA reviews). **Iter 4 (preserved):** record-propagation pass. This revision (System Analyst): **no specification content changes** — zero findings and zero CRs target this artifact across all three full-coverage review cycles (Reviewer verdicts: Iter 1 Approved; Iter 2 Approved, zero findings; Iter 3 Approved, clean — A-26 fourth-clause propagation verified); the Document Control milestone record is updated to the observed validation state so this artifact does not lag the delivery its contracts specify (the same record-propagation class the Reviewer flagged on other artifacts). **Requirements Specifier contribution (Iter 4):** the REL-002/REL-003 threshold rows in §Reliability carry the Observed annotation (R004 drop simulation, Elaboration Iter 3, CI run 33617748483) plus a validation-status paragraph — record propagation into the normative table owned by this role per the recorded stakeholder decision delegating threshold quantification; threshold values, testable criteria, and contracts unchanged; the `[ASSUMPTION]` tags (derivation bases) remain in place; formal AC-005 acceptance test remains Construction Iter 1. **Iter 3 (preserved):** convergence cycle continuation. This revision (System Analyst — **A-26, fourth behavioural-bar clause propagation**, stakeholder contribution at the Iter 2 verdict gate, binding): **the R001 behavioural-bar reliability contract extended from three to four clauses** — clause (d), verbatim: "a missing attribute is displayed as missing. It is never replaced by a default, a placeholder, a guessed value, or another employee's value"; four consumers unchanged (UC-004/005/006/007); component diagram updated; testability note extended (substitution-attempt fixtures so the fourth clause can actually fail); Traceability rows updated. **Iter 2 (preserved):** Reliability section extended with the "R001 Behavioural Bar — LDAP attribute absence" subsection (stakeholder decision, Elaboration Iter 2) — one reliability contract at the LDAP Query Mechanism boundary with four consumers; component diagram added; distinct-condition boundary vs AD-unreachable documented; bar reach stakeholder-confirmed (answer "Yes"). All FURPS+ sections, quantified thresholds, cross-cutting mechanisms diagram, and traceability rows preserved exactly as reviewed. The untagged >90% R001 acceptance criterion flagged as Risk List F1 (Reviewer) lives in the Risk List / SAD PoC Plan / Test Evaluation Summary / TC-011 — it is verified ABSENT from this artifact, and the Iter 2 revision records its replacement (behavioural bar) in the requirements baseline. **Iter 1 (preserved):** offline clocking [SCOPE_QUESTION] RETIRED; Data Integrity section added (DAT-001, DAT-002); SEC-006/SEC-007 added; INT-005 added; Offline Sync Mechanism added to cross-cutting mechanisms diagram; thresholds quantified (PRF-001/002/003, REL-001/002/003); timestamp [SCOPE_QUESTION] RETIRED (store UTC, display office local, export ISO-8601 with explicit offset, payroll day = local calendar day); office local timezone [SCOPE_QUESTION] RETIRED (America/Havana, IANA, DST-aware) |
| Elaboration Changes | **Iter 7 (this revision):** no requirement, threshold, or contract changed — the only delta is the Document Control milestone record correction (SUP F1 / A-45, post-write staleness subclass): the stale remainder-enumeration (which named A-40/A-41 as remaining after both landed and ledger-closed 2026-09-03) and the stale census claim (zero open SCM issues vs the live 1 open CR vehicle) corrected, and the remainder replaced by an owner-reference per the stakeholder's Iter 6 directive so this record cannot drift again; all FURPS+ sections, the four-clause R001 contract, and the REL-002/REL-003 observed-status rows preserved verbatim. **Iter 6 (preserved):** no requirement, threshold, or contract changed — the only delta is the Document Control milestone record's remainder-enumeration corrected to the observed state (same-pass discipline, R014 mitigation; Requirements Specifier); all FURPS+ sections, the four-clause R001 contract, and the REL-002/REL-003 observed-status rows preserved verbatim. **Iter 4 (preserved):** no requirement, threshold value, or contract changed — all FURPS+ sections, the four-clause R001 contract, cross-cutting mechanisms diagram, and traceability rows preserved verbatim (zero findings, zero CRs; A-26 verified complete at the Iter 3 review). The only content delta is the Requirements Specifier's record propagation: the REL-002/REL-003 quantified-threshold rows in §Reliability now carry their OBSERVED validation status (R004 drop simulation — zero duplicates, zero losses, sync ≤ 60 s, confirmations < 1 s; CI run 33617748483; the Test Case Cycle 1 formal-pass record is the authority — no result claimed beyond it), and a validation-status paragraph distinguishes the observed mechanism behavior from what remains unobserved (formal AC-005 acceptance test, Construction Iter 1; production-instance behavior, R010/R011, Construction Iter 3). The `[ASSUMPTION]` derivation-basis tags remain in place — they record the basis of the threshold values, which is unchanged. Document Control milestone record updated to the observed validation state. **Iter 3 (preserved):** FOURTH behavioural-bar clause propagated (stakeholder contribution at the Iter 2 verdict gate — binding; tracked as A-26 in the Review Record): the R001 reliability contract is now **FOUR clauses** — (a) every employee is rendered whether or not their attributes are complete; (b) a missing attribute never removes someone from search results; (c) a missing attribute never raises an error; (d) a missing attribute is displayed as missing — never replaced by a default, a placeholder, a guessed value, or another employee's value. Stakeholder rationale, verbatim: "Blank is an answer. 'General', or the first office in the list, is a fabrication — and on the CSV that reaches payroll a fabricated department is worse than an empty cell. An empty cell gets questioned. A plausible wrong one does not." The first three clauses stop data from being LOST; the fourth stops it from being INVENTED. **Iter 2 (preserved):** R001 behavioural bar recorded (stakeholder decision, Elaboration Iter 2): behavioural, not statistical; >90% figure dropped (invented, unsourceable); real-AD data-quality measurement moves to Construction (R011 residual, STK-004-dependent), excluded from the LCA evidence package; bar reach stakeholder-confirmed (answer "Yes"): all four AD-reading use cases. **Iter 1 (preserved):** offline clocking [SCOPE_QUESTION] RETIRED; Data Integrity section added (DAT-001, DAT-002); SEC-006/SEC-007 added; INT-005 added; Offline Sync Mechanism added to cross-cutting mechanisms diagram; thresholds quantified (PRF-001/002/003, REL-001/002/003); timestamp [SCOPE_QUESTION] RETIRED (store UTC, display office local, export ISO-8601 with explicit offset, payroll day = local calendar day); office local timezone [SCOPE_QUESTION] RETIRED (America/Havana, IANA, DST-aware) |
## Functionality
...
| REL-002 | Offline tolerance: network disruptions of at least 5 minutes are tolerated; per-client queue capacity ≥ 10 clocking events per employee browser; queued events are never lost; idempotent conflict policy — an exact duplicate (same employee, same event type, same recorded timestamp) is rejected, never duplicated; events ordered by recorded timestamp, not arrival order | Simulate a 5-minute outage: events queue locally with confirmation shown from queued data; after restore, all queued events are persisted with zero duplicates and zero losses | AC-005 (declared 5 min); queue capacity [ASSUMPTION — requires validation] — at most 2 transitions per employee per workday (FR-004 in/out), so 10 events covers 5 full workdays of total outage; conflict policy per DAT-001. **Observed (Iter 4 record propagation):** exercised and holding in the R004 drop simulation (Elaboration Iter 3) — zero duplicates (double replay AND mixed online+queued), zero losses, recorded-order preservation; execution trace CI run 33617748483; formal AC-005 acceptance test remains Construction Iter 1 (Iteration Plan) |
| REL-003 | Sync completion: all queued events persisted ≤ 60 seconds after connectivity is restored | After restore, measure time from reconnection to full persistence of the queued set: ≤ 60 s | [ASSUMPTION — requires validation] — worst case 200 employees × 1 queued event each (STK-003 population), small records, restored corporate LAN. **Observed (Iter 4 record propagation):** sync ≤ 60 s measured PASS in the R004 drop simulation (Elaboration Iter 3; execution trace CI run 33617748483); formal AC-005 acceptance test remains Construction Iter 1 (Iteration Plan) |
...
**Validation status (Elaboration Iter 4 record propagation — Requirements Specifier, threshold owner per the recorded stakeholder decision):** the REL-002/REL-003 thresholds this role quantified are no longer unvalidated assumptions about mechanism behavior — the R004 drop simulation (Elaboration Iter 3) exercised them and they held: zero duplicates, zero losses, sync ≤ 60 s, confirmations < 1 s, recorded-order preservation (execution trace CI run 33617748483; the Test Case Cycle 1 formal-pass record is the authority — no result is claimed beyond it). The `[ASSUMPTION]` tags REMAIN in place: they record the derivation basis of the threshold values (AC-005 + STK-003 population reasoning), which is unchanged and Reviewer-verified compliant. What remains unobserved is the formal AC-005 acceptance test against the deployed system — Construction Iter 1 per the Iteration Plan — and production-instance behavior (R010/R011, Construction Iter 3). REL-001's availability window is an operational commitment measured in Transition, not a mechanism threshold; no validation status is appended to it.
...
| R001 behavioural bar (stakeholder decision, Elaboration Iter 2; fourth clause added at the Iter 2 verdict gate, propagated Iter 3) | Declared R001 + stakeholder answer (Elaboration Iter 2): the bar is behavioural, not statistical — every employee is rendered whether or not their attributes are complete; a missing attribute never removes someone from search results; a missing attribute never raises an error; + stakeholder contribution at the Iter 2 verdict gate (fourth clause, verbatim): a missing attribute is displayed as missing — never replaced by a default, a placeholder, a guessed value, or another employee's value; gaps seeded deliberately in the disposable LDAP directory, including substitution-attempt fixtures so the fourth clause can actually fail; the prior >90% statistical criterion is dropped (invented, unsourceable); real-AD data-quality measurement moves to Construction (R011 residual, STK-004-dependent), excluded from the LCA evidence package | Authorizes | UC-004 AF-2 (declared home — FR-010); UC-005 AF-3, UC-006 AF-3, UC-007 AF-3 (stakeholder-confirmed, Elaboration Iter 2 — asked whether the bar applies to all four AD-reading use cases and not only the directory search, the stakeholder answered "Yes"); R001 PoC (Architect, Work Item 7 — deliberately-seeded gaps + substitution-attempt fixtures); Test Case TC-011 + TC-021/022/023 (fourth-clause verification steps per A-28) |
| R001 behavioural bar — distinct-condition boundary | R001 behavioural bar (stakeholder decision, Elaboration Iter 2; extended Iter 3) + CON-006 | Refines | AD-unreachable flows (UC-004 AF-3, UC-005 AF-2, UC-006 AF-2, UC-007 AF-2) — a different failure mode with a different contract; NOT waived by the behavioural bar |
