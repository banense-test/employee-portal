...
| Timestamp convention | Clocking timestamps are stored in UTC, displayed in the office local timezone, and exported in ISO-8601 with an explicit offset; the payroll day is the local calendar day, never the server's. All 3 offices share one timezone (stakeholder-confirmed). | Verify a recorded event end-to-end: stored value is UTC; displayed value is office local time; exported value carries an explicit offset; month-boundary grouping follows the local calendar day | DAT-001; stakeholder decision (Elaboration Iter 1); office local zone identifier: [SCOPE_QUESTION — see decision paragraph below] |
...
**[SCOPE_QUESTION — not declared, consequential: the office local timezone identifier.]** The decided convention requires the actual zone identifier to be implementable and testable: display conversion (UTC → office local), the explicit offset in every exported payroll row (FR-002, UC-006 event_timestamp), daylight-saving behavior, and the local-calendar-day month boundary all depend on the zone value. The declared input names no office locations (stakeholder-confirmed), so the zone cannot be derived from scope and must not be invented. **Escalated to the stakeholder this iteration via REQUIRES_USER_INPUT.** Until answered: the mechanism is decided and unblocked (UTC storage, local display, offset export — the Architect and Designer can build the timezone-aware mechanism against a configurable zone); only the zone value is pending, and the timestamp-convention testable criteria above cannot be executed without it.
...
| Office local timezone identifier [SCOPE_QUESTION] | Stakeholder decision (Elaboration Iter 1) — convention decided; zone value not in declared input | Refines | UC-001 timestamp convention; UC-006 event_timestamp column — pending stakeholder answer (escalated in-round) |
