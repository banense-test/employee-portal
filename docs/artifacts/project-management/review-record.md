...
**Stakeholder contribution at the verdict gate (Iter 5 — EMITTED at the close of this consolidation; answer to be folded on receipt):** the contribution question is emitted at the close of this consolidation pass in valid emission format (the marker on its own line, immediately followed by the minimal JSON array — question/type/isRequired only, per the recorded parser lesson). On receipt, the answer is folded into this Review Record (Document Control's Iter 5 coordinator row — the established home for folded verdict-gate answers, Iter 4 precedent), verified against the artifacts before folding (not deferred blindly), and the milestone auto-iteration (requiresIteration: TRUE) is recorded immediately after the folding. The contribution does not change the verdict unless it adds new scope, a correction, or a priority — the same three conditions (1 Major open, planned scope incomplete, sanction REFUSED standing) that require iteration before the answer require it after; the contribution gate closes the cycle without adding work unless the stakeholder adds work. The next stakeholder touchpoint after the contribution gate is the R6 fresh sanction request itself (owned by the Review Coordinator + Management Reviewer).
