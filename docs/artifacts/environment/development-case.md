...
|---|---|
| Phase | Elaboration |
| Status | Draft — Elaboration Iter 7 evolution; **Development Case F6 (Minor) RESOLVED this revision (A-43 landed)**: the Milestone Target's two stale items corrected to the observed state — item (1): A-40 LANDED and TES F4 ledger-closed 2026-09-03; item (4): SCM Issues #11/#12 CLOSED cr:complete (verified in the live SCM census this pass) — and the "what remains" enumeration is replaced by the single-owner reference (binding stakeholder directive, Iter 6 verdict gate — adopted this revision); the remainder re-verified against the verified findings ledger AND the live SCM census immediately before upsert, per this DC's own binding same-pass record-propagation discipline; closure owned by the emitting Reviewer lens; submitted for the R6-track review |
| Milestone Target | End of Elaboration (LCA) — NOT yet achieved; sanction still withheld per the stakeholder's binding all-findings directive (reinforced at the Iter 5 verdict gate, verbatim: "No, please fix all findings"). The R6 evidence package is ASSEMBLED and internally consistent — its core (the PoC observed-results ledger, A-32) landed and ledger-closed at Iter 4; the R014 record-correction cycle delivered both of its named corrections (A-40 TES, A-41 DC — landed and ledger-closed 2026-09-03). **Remaining-work state (single-owner rule, binding — stakeholder Iter 6 verdict-gate directive): the live state of open findings and open SCM issues is OWNED by the Review Record findings ledger and the SCM issue tracker; this Development Case REFERENCES those owners and does not restate their contents — a document that copies someone else's state drifts from it forever.** This DC's own remaining obligation: participation in the R6 re-presentation (empty findings ledger + evidence package + fresh sanction request to STK-001) once the ledger's owners close their entries. This revision resolves the DC's own open finding (DC F6 — A-43); no other open finding targets this artifact (re-verified against the findings ledger immediately before upsert) |
| Iteration | 7 (Cycle 1) |
| Date | 2026-09-03 |
| Prior Review | Inception (LCO: GO) and Elaboration Iters 1–5: zero findings on the Development Case at Iter 1; F1 (Major, A-17) + F2 (Minor, A-20) raised at Iter 2, RESOLVED at Iter 3; F3 (Minor, A-36) raised at Iter 3, RESOLVED at Iter 4 (ARCH-6 gap flag closed on verification); F4 (Minor, A-39) raised at Iter 4, RESOLVED at Iter 5 (the three stale A-32/PM-close-pass status claims corrected to the observed state); F5 (Minor, A-41) raised at Iter 5, RESOLVED at Iter 6 (the Milestone Target's stale A-37/A-38 enumeration corrected to the observed state; ledger-closed 2026-09-03). Elaboration Iter 6 (technical lens): **1 finding — F6 (Minor, A-43): the Milestone Target's items (1) and (4) stale vs the observed state (A-40/TES F4 landed and ledger-closed 2026-09-03; SCM Issues #11/#12 CLOSED cr:complete) — the post-write staleness subclass of the R014 class. RESOLVED this revision** — both items corrected AND the enumeration replaced by the single-owner reference, the terminal cure the stakeholder directed at the Iter 6 verdict gate. Valid content otherwise preserved |
| Governance Re-recorded (Elab Iter 7) | DC §4 classification: unchanged — not business-process-led (re-recorded 2026-09-03 via record_dc_classification; 5 criteria cited; Business Reviewer lens sustained BR-OK-INACTIVE at six consecutive LCA-cycle reviews, Iters 1–6). Version policy: unchanged — .NET 10 framework pin (CON-001); no new declared versions (re-recorded via record_version_policy). Optional triggers: **unchanged — Architectural Proof-of-Concept FIRED** (1 of 6, re-verified via record_optional_artifact_triggers; the artifact is APPROVED — the R6 evidence package's core; its record obligations are DISCHARGED: A-32 landed and ledger-closed at Iter 4, the A-38 sha citation landed and ledger-closed at Iter 5; no open finding targets it — re-verified against the findings ledger this pass) |
| Stakeholder Decisions Incorporated (cumulative; Elab Iter 7 revision) | (1) R001 validation bar is behavioural, not statistical — the unsourced >90% figure is dropped; production-AD data-quality measurement moves to Construction. (2) The behavioural bar applies to all four AD-reading UCs (UC-004, UC-005, UC-006, UC-007). (3) Featured-news rendering contract: **featured banners STACK, ordered newest first — every featured item renders its own banner** (stakeholder answer "newest first"; faithful record per Design Model P-02). (4) Binding directive from the Iter 1 LCA review: fix ALL findings including Minors before phase transition — adopted as exit criterion 9. (5) FOURTH behavioural-bar clause, verbatim: "a missing attribute is displayed as missing. It is never replaced by a default, a placeholder, a guessed value, or another employee's value." — added to all four AD-reading UCs; the R6 evidence gate requires FOUR-clause × four-consumer R001 evidence. (6) The Implementer's two-iteration code absence was stakeholder-attributed to a technical problem beyond its control; the code push was the stated priority for Iter 3 — FULFILLED and verified (3 mechanisms merged, PRs #3/#4/#5 + #6 APPROVED, formal TC pass complete, Issue #1 closed). (7) Iter 3 R6-path confirmation: the stakeholder confirmed the path — record corrections, then the R6 re-presentation with the evidence package and a fresh sanction request — plus one binding framing directive, verbatim: "the 8 BLOCKED test cases are a recorded SCOPE decision (production AD and Keycloak integration belongs to Construction), not an open gap. State it that way in the evidence package so the LCA reads them as deferred, not as missing." (8) Iter 4 verdict gate: "Close all findings and issues opened" — reinforces the standing all-findings directive and extends it to the open SCM issues; Issue #9 (the PoC results-ledger CR) closed cr:complete on the verified A-32 evidence; Issues #1/#2 already closed. (9) Iter 5 verdict gate: "No, please fix all findings" — the "No" closes the contribution question (nothing to add for the next pass) and "please fix all findings" REINFORCES the standing all-findings directive for the R014 record-correction cycle's remainder (TES F4 → A-40; DC F5 → A-41; F-CR-E3-1/2 carried with their recorded Construction-scope dispositions); the answer adds NO new scope, NO correction, NO priority beyond the standing record. **(10) NEW (Iter 6 verdict gate) — the single-owner directive, verbatim:** "Stop remediating this artifact one location at a time. The defect is not the enumerations — it is that the Test Evaluation Summary states what remains in five places. Four consecutive iterations have fixed some copies and missed others (F1, F2, F3, F4, and now #14). It will not converge. State the remaining work ONCE, in a single section that owns it. Every other place that needs it — Milestone Target, the master-workflow diagram, the schedule — references that section instead of restating it. And consider whether the TES should enumerate remaining actions at all: that state already has an owner in the Review Record findings ledger and in the SCM issues. A document that copies someone else's state will drift from it forever." — **adopted as the single-owner rule (binding) in § Guidelines and Procedures; applied to this DC's own Milestone Target this revision (the DC F6 terminal cure)** |
## Tailoring Overview
...
Recorded via `record_optional_artifact_triggers` (Elaboration Iter 7, 2026-09-03): `["Architectural Proof-of-Concept"]`. This replaces the prior iteration's set — the whole set is re-evaluated every iteration.

```plantuml
@startuml
!theme plain
title Employee Portal — DC §5.2 Optional Trigger Re-evaluation (Elaboration Iter 7 — single-owner adoption cycle)

start
:Load current phase, Risk List,
and project facts (Iter 7 re-check);
...
if (Architectural Proof-of-Concept condition still holds?) then (holds)
  ...
  note right
    Condition re-verified (Elab Iter 7):
    Elaboration phase (YES — the phase is
    open: LCA sanction withheld, R6
    re-presentation pending) AND at least
    one technical risk requiring empirical
    validation per Risk List (YES — R001
    HIGH, exposure 9; R003, R004 — all
    three empirically VALIDATED and
    RETIRED (Elaboration scope), the
    retirement RECORDED in the Risk List
    close-pass reappraisal).
    Stakeholder decision (Elab Iter 1,
    binding): produced in Elaboration AND
    validated empirically — SATISFIED and
    RECORDED (formal TC pass 15 PASS /
    0 FAIL / 8 BLOCKED, trace CI run
    33617748483; R001 FOUR clauses x FOUR
    consumers PASS).
    The artifact is APPROVED — the R6
    evidence package's core. Its record
    obligations are DISCHARGED: A-32
    landed and ledger-closed (Iter 4);
    the A-38 sha citation landed and
    ledger-closed (Iter 5). No open
    finding or correction targets it
    (re-verified against the findings
    ledger and the live SCM census this
    pass).
  end note
else (not held)
...
| Business Modeling INACTIVE | DC §4 classification (re-recorded Elab Iter 7, verdict unchanged; BR lens sustained BR-OK-INACTIVE at Iter 1–Iter 6 LCA reviews — six consecutive verifications) | Derives | record_dc_classification(isBusinessProcessLed=false) |
| PoC trigger FIRED | DC §5.2 condition + R001 (Risk List) + Elaboration phase (re-recorded Iter 7 via record_optional_artifact_triggers) | Derives | record_optional_artifact_triggers(["Architectural Proof-of-Concept"]); SoftwareArchitect (owner); PoC artifact (APPROVED — the R6 evidence package's core; record obligations DISCHARGED: A-32 landed and ledger-closed Iter 4; A-38 sha citation landed and ledger-closed Iter 5; no open finding targets it — re-verified this pass) |
...
| Behavioural bar scope — four AD-reading UCs (stakeholder confirmation, Elab Iter 2) | Stakeholder answer: the bar applies to UC-004 (blank fields), UC-005 (blank display fields), UC-006 (blank CSV cells, no abort), UC-007 (locatable and selectable) — fourth clause added to all four | Authorizes | Use-Case Model (UC-004..UC-007 alternative flows — propagated, verified); Supplementary Specification; Design Model rendering contracts; CONTRIBUTING.md ARCH-6 (four-clause — re-verified this iteration); PoC artifact |
...
| Iter 4 verdict-gate answer (stakeholder decision, Elab Iter 4) | Stakeholder answer, verbatim: "Close all findings and issues opened" — reinforces the standing all-findings directive and extends it to the open SCM issues; Issue #9 (the PoC results-ledger CR) closed cr:complete on the verified A-32 evidence; Issues #1/#2 already closed | Authorizes | The final record-correction pass (A-37…A-39 + PM pass-close reconciliation); the R6 entry gate (empty ledger + closed issues + evidence package + fresh sanction request) |
| Iter 5 verdict-gate answer (stakeholder decision, Elab Iter 5) | Stakeholder answer, verbatim: "No, please fix all findings" — the "No" closes the contribution question (nothing to add for the next pass) and "please fix all findings" REINFORCES the standing all-findings directive for the R014 record-correction cycle's remainder (TES F4 → A-40; DC F5 → A-41; F-CR-E3-1/2 carried with their recorded Construction-scope dispositions); the answer adds NO new scope, NO correction, NO priority beyond the standing record | Authorizes | The R014 record-correction cycle (A-40/A-41 + PM pass-close reconciliation); the R6 entry gate (empty ledger + evidence package + fresh sanction request) |
| Iter 6 verdict-gate answer — the single-owner directive (stakeholder decision, Elab Iter 6 — NEW this revision) | Stakeholder answer, verbatim: "Stop remediating this artifact one location at a time. The defect is not the enumerations — it is that the Test Evaluation Summary states what remains in five places. Four consecutive iterations have fixed some copies and missed others (F1, F2, F3, F4, and now #14). It will not converge. State the remaining work ONCE, in a single section that owns it. Every other place that needs it — Milestone Target, the master-workflow diagram, the schedule — references that section instead of restating it. And consider whether the TES should enumerate remaining actions at all: that state already has an owner in the Review Record findings ledger and in the SCM issues. A document that copies someone else's state will drift from it forever." | Authorizes | The single-owner rule (binding — § Guidelines and Procedures, adopted this revision); the DC F6 terminal cure (the Milestone Target's remainder enumeration replaced by the owner reference); the R6 entry gate (empty ledger + evidence package + fresh sanction request) |
| All-findings-closure exit criterion (criterion 9) | Stakeholder directive (Iter 1 LCA review, verbatim): fix all findings, including minors, before moving to the next phase; reinforced at the Iter 4 verdict gate ("Close all findings and issues opened") and at the Iter 5 verdict gate ("No, please fix all findings"); the Iter 6 verdict gate adds the single-owner directive (the structural cure for the class that keeps minting the remainder findings); Review Record Iteration Plan F4 (Major, A-12 — done) | Derives | Elaboration phase transition sanction; findings-ledger verification at each iteration close |
| Record-propagation discipline (adopted Iter 4; applied to this DC at Iter 5, Iter 6, and Iter 7) | Review Record (Elab Iters 3–4): 9 findings across two passes, ALL record-propagation class (records lagging observed delivery); F7 status-honesty lesson (both directions) | Derives | Every artifact carrying a record of observed state (PoC results ledger, SAD criterion 3, TC summary, TES verdict, Iteration Plan statuses, Risk List retirement rows, this DC) — updated in the SAME pass as the state change |
| Self-propagation containment (adopted Iter 5; enforced Iter 6; TERMINALLY CURED at Iter 7) | Review Record (Elab Iter 4 defect distribution): the record-propagation class is SELF-PROPAGATING — each pass's landings stale the prior pass's remainder-enumerations (TES F3 Major, PoC F3/DC F4 Minor born of the pass's own success); the Iter 5 review minted its successor instance (TES F4 Major, DC F5 Minor) exactly as the Risk List R014 registered; the Iter 6 review minted 5 successor Minors (all post-write staleness — census/remainder currency only) | Derives | All remainder-enumerations in every artifact — written against the CURRENT pass's landings and re-verified against the verified findings ledger before upsert; **the class's terminal cure is the single-owner rule (Iter 7): a reference cannot go stale** |
| Single-owner rule (binding — ADOPTED this revision, Iter 7) | Stakeholder Iter 6 verdict-gate directive (verbatim — see the decision row above); measured basis: five iterations of remainder-enumeration corrections (TES F1→F3→F4→F5; DC F4→F5→F6), each pass's landings staling the prior pass's copies | Derives | All artifacts' remaining-work statements — state ONCE in a section that owns it (or do not enumerate at all); every other location (Milestone Target, master-workflow diagram, schedule) REFERENCES the owner (Review Record findings ledger + SCM issue tracker) instead of restating it |
...
| Framework pin .NET 10 | CON-001 | Derives | record_version_policy(framework, .NET, 10) — re-recorded this iteration (Iter 7); no new declared versions |
| Elaboration Iter 7 entry criteria | Review Record (Elab Iter 6: NO-GO CONFIRMED on the post-write staleness remainder, requiresIteration=TRUE; the single-owner directive received at the verdict gate; A-42…A-46 + PM pass-close assigned; R6 path stakeholder-CONFIRMED) | Refines | Single-owner adoption-cycle execution (A-43 + PM pass-close reconciliation; A-42/A-44/A-45/A-46 owned by their named roles) |
| LCA exit criteria (1–9) | RUP LCA milestone criteria + SAD §LCA Review + stakeholder decisions (empirical PoC — OBSERVED and RECORDED; FOUR-clause behavioural bar — OBSERVED; all-findings directive; R6-path confirmation; Iter 4 + Iter 5 + Iter 6 verdict-gate answers) | Refines | End-of-Elaboration milestone gate (re-presentation at R6) |
...
| Marker-retirement discipline | Stakeholder decisions: offline mechanism, timestamp convention, America/Havana, PoC empirical scope, R001 behavioural bar (+ four-UC scope), featured-news contract, FOURTH behavioural-bar clause, R6-path confirmation + framing directive, Iter 4 verdict-gate answer, Iter 5 verdict-gate answer, Iter 6 verdict-gate answer (single-owner directive) | Authorizes | Use-Case Model, Supplementary Specification, SAD, this document (markers retired in-place) |
| CI verification | `.github/workflows/ci.yml` (SCM read, Iter 2); main GREEN run 33658332611 (per the Code Reviewer lens's same-day Iter 6 verification) | DependsOn | Implementer, ConfigurationManager |
| Deploy skeleton verification | `.github/workflows/deploy.yml` (SCM read, Iter 2) | DependsOn | DeploymentManager (Construction), R010 |
| CONTRIBUTING.md closure + ARCH-6 verification (gap CLOSED; re-verified this iteration — Iter 7) | SCM reads (sha `90e4f2e…`, this iteration — ARCH-6 four-clause verified first-hand, unchanged from Iter 4); Review Record F-CR-E1-2 / SCM Issue #2 (A-5 — closed); Review Record DC F3 (A-36 — resolved Iter 4) | DependsOn | Code Reviewer CR-1 (citable four-clause rule baseline); Construction code reviews |
...
| Lint gap (.editorconfig, Directory.Build.props) | Tool assessment (SCM reads — not found, re-verified this iteration — `.editorconfig` not found on main) | DependsOn | Implementer (owner); non-blocking, explicitly deferred with rationale — carried to Construction Iter 1 |
...
| Measurement actuals note | Work Order Measured Actuals (Inception: 28 min, 1,347,939 tokens, 11 runs, 10 artifacts; Elaboration Iter 3: 3.6 h agent, 0s queue, 27,143,633 tokens, 22 runs, 13 artifacts; Elaboration Iter 4: 3.0 h agent, 0s queue, 24,830,875 tokens, 23 runs, 13 artifacts; Elaboration Iter 5: 4.7 h agent, 0s queue, 25,184,977 tokens, 23 runs, 13 artifacts; Elaboration Iter 6: 6.9 h agent, 0s queue, 27,272,284 tokens, 22 runs, 13 artifacts) | Refines | ProjectManager (budget-box basis); Process Engineer (process-bottleneck analysis) |
...
| Development Case F4 resolution (A-39, Iter 5 revision) | Review Record (Reviewer lens, Elab Iter 4): DC F4 Minor — three stale A-32/PM-close-pass status claims (exit-criteria criterion 3 "Remaining" line; PoC disposition paragraph + trigger-diagram note; Organization Assessment) lagging the same-pass observed landings; remediation: Process Engineer updates the three locations to the observed state, per the DC's own binding same-pass record-propagation discipline | Resolves | This document (all three locations corrected: criterion 3 "Remaining" line → the record-propagation obligations are discharged; PoC disposition + trigger-diagram note → obligation DISCHARGED, A-32 landed and ledger-closed; Organization Assessment → retirement RECORDED at the PM close-pass); Reviewer lens (closure via resolve_artifact_finding — ledger-closed Iter 5) |
| Development Case F5 resolution (A-41, Iter 6 revision) | Review Record (Reviewer lens, Elab Iter 5): DC F5 Minor — the Milestone Target enumerates A-37/A-38 as remaining while both landed and ledger-closed that same pass (only A-39 correctly marked resolved) — the R014 self-propagation class, second location; remediation: Process Engineer corrects the Milestone Target to the observed state, with the remainder re-verified against the verified findings ledger before upsert, per the DC's own binding same-pass discipline | Resolves | This document (Milestone Target corrected to the observed state); Reviewer lens (closure via resolve_artifact_finding — ledger-closed 2026-09-03, the A-41 landing verified first-hand at the Iter 6 review) |
| Development Case F6 resolution (A-43, this revision) | Review Record (Reviewer lens, Elab Iter 6): DC F6 Minor — the Milestone Target's items (1) and (4) stale vs the observed state (A-40/TES F4 landed and ledger-closed 2026-09-03; SCM Issues #11/#12 CLOSED cr:complete) — the post-write staleness subclass of the R014 class; remediation: Process Engineer corrects the two items to the observed state, per the DC's own binding same-pass discipline | Resolves | This document (both items corrected AND the remainder enumeration replaced by the single-owner reference — the terminal cure the stakeholder directed at the Iter 6 verdict gate; remainder re-verified against the verified findings ledger AND the live SCM census immediately before upsert); Reviewer lens (closure via resolve_artifact_finding when verified) |
